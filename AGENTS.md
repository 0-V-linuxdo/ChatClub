...
- Simple destructive confirms that only need a title, body, and footer actions may call `openConfirmationAction`. That helper is not a fifth overlay family: it always opens `confirmationModal` and accepts `tone` (`danger` | `warning` | `neutral`, default `danger`), button `variant`, optional `acknowledge` copy (danger action stays disabled until checked), and optional `busyLabel`. Severity is expressed through those existing knobs, not a new family: remaining routine item deletes use `tone="neutral"` with `variant="danger"`; risky runtime or security confirms use `tone="warning"` plus `.overlay-warning-card`; critical or batch irreversible confirms keep default `tone="danger"` and may pass `acknowledge`. Reversible local list-item deletes (Prompt History items, topbar placeholders) use an undo toast instead of a confirmation. ChatClub Tabs forget stays a confirmation because it can close a live browser tab. Official-rules mutations use this helper instead of a private confirmation wrapper. Do not use `window.confirm` or native `<dialog>`.
...
Chrome tokens: `--overlay-radius` (= `--ui-radius`), `--overlay-border` / `--overlay-border-color`, `--overlay-shadow`, `--overlay-close-size`, `--overlay-width` / `--overlay-width-compact` / `--overlay-width-wide` / `--overlay-width-task` / `--overlay-width-workspace`, `--overlay-gutter` (40, typed-modal viewport inset), `--overlay-gutter-panel` (32, persistent Summary/Share and Settings resting height), `--overlay-gutter-tight` (16, popovers/tooltips), `--overlay-backdrop`, `--overlay-z-modal-nested`, `--overlay-header-height` (46, typed-modal chrome; Settings titlebar may be taller internally), `--overlay-panel-offset` (Summary rest top), `--overlay-panel-offset-raised` (Share rest top). Surfaces use `.overlay-surface`. Window close/maximize controls use `.overlay-window-button`; family classes (`settings-window-button`, `pocket-window-button`, `prompt-history-window-button`, `workspace-tabs-search-window-button`, `summary-window-button`, `share-window-button`) are JS hooks only and must also carry `.overlay-window-button`. Fill-viewport geometry uses `.overlay-surface-fullscreen` (inset 0, radius 0, no shadow), declared for both that class and `.modal.overlay-surface-fullscreen`, because `.modal` / `.modal-wide` declare their own width and height later in the sheet: with only the one-class form, Settings fullscreen measured 1120×680 in a 1280×720 viewport (`inset: 0` applied while the wide width still governed). A modal family that overrides that width states it as a two-class rule (`.modal.settings-modal`, `.modal.workspace-tabs-search-modal`); a one-class family rule silently loses to `.modal-wide`. Persistent-panel maximize uses `.maximized` / family aliases at inset 12; do not merge that recipe into a third fullscreen class. Persistent Summary/Share resize uses three-edge 12px `.overlay-panel-resize-handle` (family handle classes are aliases). Viewer-window resize stays four-edge `.overlay-viewer-resize-handle`. Do not resurrect `.pocket-panel-resize-handle`.

Motion: `--overlay-motion` opacity fade on transient overlays (modal backdrop, popover). Do not animate `transform` on positioned panels, and do not replay enter motion on persistent-panel redraws. Toast/tooltip enter displacement must disable under `prefers-reduced-motion`. Toast/frame-toast blur, the frame-toast position sample, and composer image-remove frost must disable under `prefers-reduced-transparency`. Layout `transform` used only to center a persistent panel is allowed; enter-motion transform is not.

Typed `modal()` must stay a `div`/`section` overlay: `role="dialog"` (or `alertdialog` for danger/warning `confirmationModal`; `tone="neutral"` keeps `role="dialog"` and the `modal-alertdialog` chrome class), `aria-modal="true"`, `aria-labelledby` on the title, `aria-describedby` on the confirmation body root (`.overlay-confirmation`, `[data-overlay-description]`, or the first `p`), scroll lock while open, Tab containment, and focus return on close. While any typed modal is open, `document.body` siblings except the topmost `.modal-backdrop`, `.toast-host`, and `#chatclub-global-tooltip` / `.global-tooltip` receive `inert`. Nested backdrops stay inert so only the top dialog is interactive. Do not use `aria-hidden` on a focused ancestor. Confirmation initial focus lands on the least-destructive footer action. Confirmation chrome uses `tone` `danger` | `warning` | `neutral` (default `danger`) via `data-overlay-tone` on `.modal-alertdialog`. Danger and warning may tint the title with `--danger` / `--warning` via `.modal-alertdialog`; both may place the existing `alert` icon beside that title (`aria-hidden` on the decorative SVG), and neutral keeps the title in `--text` and omits the icon. Header close hover on a confirmation follows the active tone instead of `--primary`. Confirmation surfaces consume `--overlay-width-compact`. Confirmation footer `.button-danger` fills `--danger` with `--on-primary` label for danger and warning tones only; `tone="neutral"` keeps the outlined danger button. Hover and active consume `--danger-hover` / `--danger-active`. Dark theme may mix those tokens against black at a lighter ratio than light theme; do not invent new token names. Applying confirmations may stamp `.is-applying` on the primary action and reuse the existing spinner keyframes; do not invent a second busy control. `.overlay-confirmation` is the helper body skin, not a fifth overlay family. `.overlay-warning-card` is the shared warning-card skin; `.io-sensitive-warning` is an alias, not a second card system. `.overlay-confirm-ack` is the shared acknowledgement row; its checkbox hit target consumes `--target-min` through `.overlay-confirm-ack-box` without enlarging the visual box. The header close control stays and is equivalent to Cancel. Do not invent a new overlay family for that. Do not add Escape-to-close here — Overlay Dismissal Policy owns that. Nested typed modals stack with `--overlay-z-modal-nested`; the nested backdrop stays transparent so dimming does not compound. Nested modals stay below popovers.

Page toasts use a document `.toast-host` with two pre-mounted live regions (`.toast-live-polite` / `.toast-live-assertive`). Visual toast items are not live regions. Appearance `toastStay` (`short` / `default` / `long`) sets the auto-dismiss window: 1.6s / 3.2s / 32s, errors double. Hover pauses auto-dismiss. An optional `actionLabel` / `onAction` may render one action control (Undo) on the visual item; that is not a close button, and actionable toasts double their stay like errors. Frame toasts keep `role="status"` / `role="alert"` and the same timing inside `.chat-frame-wrap`; a visible frame toast must accept pointer events so hover-pause works. Do not add a close button to status toasts. Global tooltip may set `aria-describedby` on the trigger while visible; tooltip bubbles consume `--overlay-radius`. Every `.popover-menu` carries `.overlay-surface`. Linear action menus may use `role="menu"` with an accessible name, `aria-haspopup="menu"` / `aria-expanded` on the trigger, and `bindLinearMenuKeyboard` (Arrow / Home / End, roving tabindex, Tab / Shift+Tab dismisses through the owner `dismiss` callback). `menuitem` must not wrap nested interactive controls. Multi-column pickers are not menus and must not claim `role="menu"`. `.overlay-window-button` and `.popover-menu .button` share a 2px `:focus-visible` ring.

Do not migrate to native `<dialog>` without an overlay-policy audit. Overlay Dismissal Policy above is closed: do not add one-off close flags. History and Tabs search may share Pocket focus/fullscreen/resize/restore window chrome through `ui/viewer-window.js`; that shared viewer-window chrome is allowed and is not drift. Shared viewer-window defaults are overlay-neutral (`viewer.fullscreen`, `--overlay-viewer-width` / `--overlay-viewer-height`, `.overlay-viewer-resize-handle`). Pocket may pass Pocket-specific size vars and button classes at its call site. Do not grow a third window-chrome dialect at either call site. History and Tabs search detail content may reuse Pocket entry/turn/slider classes (`pocket-entry`, `pocket-message`, `pocket-size-*`); that shared content skin is allowed and is not window-chrome drift. Do not extract a third card system. Settings fullscreen is an editor special case: it may consume `.overlay-surface-fullscreen` geometry only, and must not grow remember/restore, focus mode, or resize handles. Topbar Search enters composer search mode on `.prompt-shell` (the `.prompt-input-row` is an in-flow grid so chips, field, and send no longer overlay; card chrome lives on the input row (expanded compose keeps that same row chrome). In search with a visible results list, one joined card is painted by `.prompt-shell-search::before` spanning the row and results (`--prompt-search-radius`, `--overlay-border`, `--overlay-shadow`); the row and `.prompt-search-results` stay transparent with no own border or shadow and `padding-top: 0` so there is no divider seam or see-through hairline; the shell itself stays transparent so the joined card is the row+results, not the shell. `.prompt-model-gate-status` stays in the input-row accessory track even in search and must never become a results grid-row. Results stay under the input row (min 40px; inside `#composer-center-host` that row grows with content), not a second pill card or a fifth overlay family, and not a typed modal). Search rows render site favicon stacks on the left of the title through the runtime `renderFavicons` port and a muted `<time class="prompt-search-option-time">` date on the right (`viewedAt || updatedAt || createdAt || detachedAt`, via `formatWorkspaceSearchTime` — the same `toLocaleString({ month: "short", day: "numeric" })` as Tabs search); body-only FIND hits (`matchKind: "body"` from `collectWorkspaceSearchRecords`) add one muted single-line `.prompt-search-option-snippet` under an unmarked title (clipped around the first user then assistant full-text hit; markdown emphasis, code ticks, heading hashes, and `[label](url)` markers are flattened so `**bold**` does not paint); title and app hits stay one line and do not paint a snippet; empty query and Record Full Text off never set `matchKind`/`snippet`; `highlightQuery` and the FIND matcher share NFKC case-fold with original-text offsets; FIND marks (`.workspace-tabs-search-mark`, Composer and the viewer) mix `--primary` 78% into `--panel` with `--on-primary` ink and `--font-weight-medium` so snippet hits stay readable on the `.is-active` 12% row — do not inherit muted snippet color or use a 32% transparent wash; an empty query lists recency rows and must not paint `workspace.tabs.empty`; `.ui-empty-state[hidden]` and `.prompt-search-empty[hidden]` must stay `display: none` so a hidden empty-state cannot leak under recency; the result list uses an overlay scrollbar with a transparent track and no gutter. A visible `.prompt-mode-switch` on the shell is the in-field compose/search affordance; expanded compose must not `display: none` that switch. Pressed `.prompt-mode-chip[aria-pressed="true"]` fills `--primary` with `--on-primary` ink like Send (including `:hover`); the unpressed chip, plus, and clear rest ink is `--text` on transparent (same as `.top-icon-action`); hover keeps that ink and only paints `--control-hover`; do not rest them on `--muted` or fill plus/clear with `--primary`; do not restore `--control-selected` on that chip or add a track well. The `.prompt-actions-button` plus stays visible in collapsed compose (unfocused rest state) and while its `.prompt-actions-popover` is open: `bindLinearMenuKeyboard` may move focus into that menu, but that must not collapse the shell, and collapsed CSS must not `display: none` the plus or an `aria-expanded="true"` / `.prompt-actions-button-active` trigger. Search still hides plus and send. Tab while `.prompt-input` owns the caret toggles compose and search. Search uses `workspaceSearchCopy({ voice: "composer" })` as the visible field hint (`composer.search.placeholder`, or `composer.search.placeholderFullText` while Record Full Text is on), not the Tab mode copy. Composer Search and the lazy Tabs search viewer share that helper plus `formatWorkspaceSearchTime` and `highlightQuery` from `app/workspace/tab-search.js`; composer keeps chats wording, the viewer keeps titles wording, and both describe the same FIND (ChatClub Tabs, title + optional full text). Empty compose keeps `.prompt-collapsed-preview-empty` visible (nowrap + ellipsis) while focused or expanded so a long rotating placeholder does not wrap inside the native textarea `::placeholder`; hide that preview when the field has a draft, when `.prompt-shell-has-images`, or in search mode. Compose `::placeholder` stays `color: transparent` so the two layers do not double-paint. Do not JS-slice the placeholder to `TOPBAR_PROMPT_PLACEHOLDER_MAX_LEN` for display, and do not rely on textarea `::placeholder` `text-overflow: ellipsis`. Empty focused/expanded compose with no draft still sizes the textarea and `.prompt-shell` to `--prompt-collapsed-height` (`promptInputHeight` `empty`); do not copy placeholder-inflated `scrollHeight` onto `shell.style.height`, and do not grow `--topbar-height` to fit it. `:placeholder-shown` stays `white-space: nowrap` so leftover native placeholder layout cannot add a second line box. Leaving search while `.prompt-input` still holds the caret must `expandInput`. The compose draft and search query are independent buffers: entering search stashes the shared field into `state.promptText` before writing the query, and leaving search restores that draft without clearing the last query. Search mode stays single-line in the topbar slot: `enterSearchMode` must not expand the textarea, and collapse/blur must not exit search. Inside `#composer-center-host`, compose and search share the same content-sized input row (min `--prompt-collapsed-height` 40px, grow with `scrollHeight` downward from `--composer-center-top`); do not lock that row to 56px or jump to the 180px cap for a two-line draft. Expanded compose with a newline or a wrapped second line adds `.prompt-shell-stacked`: full-width `.prompt-input` above a 40px toolbar track that never contains the textarea. Search stays single-line. Empty focused/expanded compose stays on the collapsed token. The lazy `viewerModal` Tabs search viewer remains the deep-link preview from composer search. It must not `setOpen(true)` on the ChatClub Tabs sidebar. The ChatClub Tabs sidebar has no search field; FIND lives on Composer Search plus this viewer. The factory-on `enterSearchMode` shortcut (Mac ⌥⇧F / Windows Alt+Shift+F) calls that same Composer Search enter; Tab remains the only compose/search toggle; do not add a fourth sidebar search icon, a Cmd+K HUD, or `manifest.commands`. History and Tabs search titlebar search inputs stay mounted across query redraws; do not `clear()` those fields to toggle the clear button. A true row click may blur the field. While a typed modal is open, the titlebar search is the unique caret owner: iframe load and frame restore must not move focus back to `.prompt-input`, completing a chat-frame load must not clear that frame's `inert` (modal close restores it for frames that are not still loading), and `restorePromptInputFocus` must re-check `.modal` on every retry. SPA hydrate, the navigation focus guard, and child autofocus after load must not steal that caret. While no typed modal is open, a focused `.prompt-input` is the overlay-grade composer caret owner (`composer: true`, `mode: overlay`): iframe load, SPA hydrate, and child autofocus must not steal it; a trusted pointer on the chat-frame still leaves. Do not wrap `.prompt-shell` in `viewerModal` / `editorModal` / `confirmationModal` or add class `modal`. Composer stays overlay mode so it is not a leftover page caret owner, but while `composer: true` (including after a textarea remount that keeps composerInert intent) it re-arms the same page-caret lease: stamp `chatclub_page_caret_until` / `chatclub_page_caret_token` onto the outgoing document `window.name`, prepare/adopt before src, refresh after load, and re-pin on a child `{ source: "chatclub-page-caret", action: "stolen" }` `postMessage`. Overlay search without `composer: true` must not stamp those params. composerInert is intent ownership, not textarea-node lifetime: load, remount, and `body`/`html` blur must not clear that intent until a typed `.modal`, popover, or trusted pointer on a chat-frame takes over; pin rebinds a live `.prompt-input` after remount or `clear(false)` even when the owner object is null, without waiting for another click. The composer claim does not inert chat-frames or the workspace island (`.main-grid` / `.chat-card`): `inert` only blocks the user's clicks, scrolling, and hover while a site-isolated child can still focus itself, and the 2026-09-10 failure was that the inert island swallowed every click (the wrap never received the pointerdown that was supposed to leave) so the lock could not be released. Only typed modals and the leftover `mode: page` owner inert chat-frames; `beginFrameLoading` keeps its own loading inert. The parent never sees a pointerdown routed into a site-isolated chat-frame, and its `focusout` / `window` `blur` cannot tell a user click from a steal, so the child `document_start` shield reports every trusted pointerdown as `{ source: "chatclub-page-caret", action: "pointer" }` regardless of lease state; the parent accepts that message (and `stolen`) only from a `iframe.chat-frame` `contentWindow`, ignores `pointer` while a typed modal is open, and treats it like a trusted pointer on `.chat-frame-wrap`: stamp the recent frame pointer, leave (release the lease), and `frame.focus()` only when the frame is not already `activeElement`, because Chromium 149 blurs the child's editor when the parent re-focuses a frame that already holds. A new claim clears that stamp. The report lands 1–4 ms after the parent `focusout` in steady state and ~40 ms cold (measured 2026-09-10), so focus entering a chat-frame (`focusin` on the frame, `focusout` whose relatedTarget is the frame or null, `window` `blur` with `hasFocus()` true) starts a 100 ms grace (`OVERLAY_CARET_FRAME_GRACE_MS`, `scheduleOverlayCaretFrameGrace`) instead of reclaiming synchronously or on the next frame; a child `stolen` report short-circuits it. During that grace `pinOverlaySearchCaret` reports a hold for an ambiguous destination (frame, `body`, null) so the composer blur settle does not collapse the input, while a concrete destination (topbar control, selection overlay, toast) decides at once; the composer passes `onLeave` to collapse the input when a report ends the hold. If the report lands after the grace the parent has already yanked the browsing context: the parent leaves and hands the frame back, and the shield's `restorePageCaretTrustedFocus` re-focuses the element that click had focused (within 1 s of the trusted pointerdown, only while the site left focus on `body`). A frame without the shield cannot report clicks; the user leaves through parent chrome there. Do not resurrect `syncComposerWorkspaceIslandInert`. Leaving is not enough on its own: the 2026-09-10 Arc report ("click into the frame, the caret comes back to the composer") was two other parent restores that never saw the click, because a site-isolated click produces no parent pointerdown and the parent gets no `:hover` / `pointerenter` / boundary event on an out-of-process iframe either (measured in Chromium 149; an in-process same-origin iframe does deliver them, which is why a hover heuristic looks valid in a naive lab and must not be added). First, `app/prompt-focus/controller.js` keeps its initial lock (`data-p`, `pending`) until a trusted parent pointerdown/keydown and refocuses `.prompt-input` from a 50 ms restore loop plus every `focusin`, so on a fresh page whose first interaction is a click inside a frame it pulled the caret back 4 ms after the child report. Second, `armPromptFocusRestore` stamps `promptFocusRestoreGeneration` on a frame navigated while the prompt held (or while `data-p` is set), and `restorePromptInputFocus` refocuses the prompt when that frame's late `load` completes. `leaveOverlayCaretForFrame` (`ui/dom.js`) therefore deletes `promptFocusRestoreGeneration` from every `iframe.chat-frame` and re-dispatches every accepted trusted frame pointer (wrap pointerdown or child `pointer` report) as a bubbling `chatclub:chat-frame-pointer` `CustomEvent` on `document` (`CHAT_FRAME_POINTER_EVENT`); the prompt-focus controller treats that event exactly like a wrap pointerdown (`lastFramePointerDownAt`, release), and while its lock is still pending it waits `CHAT_FRAME_POINTER_REPORT_GRACE_MS` (120 ms, one window per frame focus) before reclaiming focus that sits on a chat-frame, so the report can land first; a `stolen` report or a frame focus without any report still ends in a reclaim. `app/runtime.js` calls `ensureChatFramePointerReports()` before installing the prompt-focus controller because the report listeners were otherwise installed only by the first caret claim, which a fresh page has not made. The child shield also relays a `pointer` report from a nested same-document-owned iframe (a site that keeps its editor in a nested frame) to its own parent; it relays only `pointer`, only from a frame that document owns, and only while that document is itself embedded. Do not reintroduce a parent-side hover or `userActivation` heuristic for OOPIF clicks (`navigator.userActivation.isActive` is unconditionally true under automation and unreliable as evidence). While that intent holds and a chat-frame is loading or within the post-load pin settle, pin keeps retrying without the three-frame follow cap; a parent `document.hasFocus()` miss while the iframe owns the browsing context is still stolen, not a successful hold. Overlay search without `composer: true` must not use that load-window pin. `document.activeElement === field` with `document.hasFocus()` true is not proof of a hold either. Measured in Chromium 149 on 2026-09-10 with a site-isolated child: when the child focuses itself (`focus()`, `select()`, `label.click()`, autofocus), the parent gets `focusout` first; the owner's synchronous refocus leaves the field as `activeElement`, but the browser still moves the focused frame to the child, `hasFocus()` stays true because the focused area is a descendant, and keystrokes go to the iframe. `iframe.inert` and a native `<dialog>` do not block that. In that phantom hold `:focus` stops matching on the field and `field.focus()` is a no-op (the field is already the document's focused element); only `window.focus()` moves the focused frame back, and it re-dispatches `focus` on the field before the switch completes, so `pinOverlaySearchCaret` must guard that re-entry (`overlayCaretReacquiring`). `overlayCaretPinHolds` requires `field.matches(":focus")`; a parent `window` `blur` while `document.hasFocus()` is still true is a chat-frame steal and must re-pin on the next frame (never synchronously inside the blur, Chromium ignores it there), capped so a site that refocuses itself on every blur cannot ping-pong; a `window` `blur` with `document.hasFocus()` false is the browser window losing focus and must not fight the OS. The modal search and the composer share this pin; the composer only surfaced it because a composer submit makes the child send runtime focus the site editor and the post-load model gate runs while the site autofocuses. Focus landing on `.preferred-model-selection-overlay` or a `.frame-toast` while the composer owns the caret is a steal to reclaim, not a leave (`composerCaretStolen` true, `composerCaretShouldLeave` false): that chrome is `tabindex="-1"` with pointerdown prevented, so only the selection-overlay focus guard can put focus there, and parking the caret on it swallows keystrokes until the gate settles. The child `document_start` shield evicts `[autofocus]` while the page-caret lease is active. Completing a chat-frame load clears the loading `inert` unless a typed modal or the `mode: page` owner holds; the composer hold leaves the frame interactive, and a trusted pointer on `.chat-frame-wrap` or the child's `pointer` report leaves so the click enters the chat. Completing a chat-frame load must pin that owner without waiting for an armed restore generation. Center placement reparents the same `.prompt-shell` onto `#composer-center-host.overlay-surface` at `--overlay-z-panel` with no modal-backdrop; the host uses `--composer-center-top` (`calc(var(--topbar-height) + var(--workspace-tab-row-height, 40px) + var(--space-3))`) and `translateX(-50%)` so the top edge sits below the topbar and tab row while height grows downward. Both placements are the same width through one `:root` token, `--composer-width` (`clamp(min(320px, calc(100vw - var(--overlay-gutter-panel))), 48vw, 760px)`): the host consumes it as `width`, and `.composer` plus `.topbar-edit-slot-composer` carry it as `flex: 0 1 var(--composer-width)`. The 2026-09-15 report was that the same field measured 366–1104px in the bar while the popup stayed at 760px, because the slot resolved `flex: 1 1 min(58vw, 760px)` against whatever icons and flex spacers the user had configured while the host carried its own literal, and `@container chatclub-topbar` floors (360px / 260px) had already killed the slot's own `min-width: 420px`. Do not restore that share, those floors, or a second literal at either call site. The token is a flex basis only in the bar: a definite `width` on the slot enters the topbar's intrinsic minimum, so the bar overflows the viewport and `.app-shell` clips the right-hand controls instead of letting the pill shrink. `flex-shrink: 1` with `min-width: 0` stays as that overflow valve, so a bar too crowded to give the token still narrows the pill instead of clipping; a live `.topbar-flex-space` is therefore `flex: 1 1 0` (no basis, `min-width: 0`) so an empty spacer cannot claim room from the token while the bar is full, and `.topbar-edit-slot-flex` keeps its own draggable 54px/24px size. Below `body { min-width: 980px }` the bar is clipped either way; that predates the token. Center placement keeps that same slot in the bar and centers the 34px mark inside it (`display: flex`, `justify-content: center`, `min-width: var(--ui-chrome-height)`), so reparenting the shell moves no other topbar item. The 2026-09-15 report was that a collapsing slot (`flex: 0 0 auto`, `width: auto`) released `--composer-width - 34px` to `.topbar-flex-space`, and because the bar has no cluster anchor, every control between the composer and a spacer slid by that width times the share of spacers still to its right: the right-hand cluster moved 155px at a 1043px viewport with three spacers, and the shipped default layout, whose only spacer follows six buttons, moved those six by 433px. Do not collapse that slot, redistribute the freed width to one neighbouring spacer, or right-anchor a cluster with `margin-left: auto` to compensate. The host starts hidden unless pinned (default unpinned, session-only). An in-row `.prompt-pin-button` is `display: none` in the topbar slot and only shown inside `#composer-center-host`; pressed uses filled primary (`--on-primary` on `--primary`) and keeps the popup visible; unpinned outside pointerdown (including `chatclub:chat-frame-pointer`) hides the host without a backdrop. While the host is hidden in center placement, the required topbar `.composer-center-mark` is an icon-only `top-icon-action` (`keyboard`, no visible sentence) whose tooltip carries `focusInput`; it opens the host through `focusInput`; do not add a floating page launcher. Collapsed composer height is 40px inside `#composer-center-host` (`PROMPT_COLLAPSED_HEIGHT` / `--prompt-collapsed-height`). The topbar slot uses 38px so `6px` padding plus the pill plus the 1px border fill the 51px `--topbar-height`; do not inherit the overlay 40px into `.topbar .composer:not(.composer-center-slot)`. The 2026-09-10 stretched-header regression was the reverse direction: the composer restyle grew the topbar pill to 56px and then lifted `--topbar-height` 51 → 61 → 69 (`ae6bdca`, `3b215d7`) to contain it, while every other topbar control stayed 34px. Size the pill to the bar; never grow `--topbar-height` to fit a taller pill, and do not resurrect the model-gate `calc(51px + …)` variant. Row controls inside the pill (`.prompt-mode-chip`, plus, clear, preferred-model gate, send, pin) are all `--ui-accessory-height` so they fit the 38px slot with 4px insets; do not raise the send back to `--ui-compact-height`. `.composer { height }` follows the inherited token. Edit mode always keeps the shell in-bar. That host is not a fifth overlay family, and its overlay-surface class is a placement hook that stays visually transparent so it is not a wrapping card. The preferred-model gate visual (`.prompt-model-gate-status`) is a child of `.prompt-input-row` immediately before `.prompt-send-button` (clear | GATE | send | pin), so `applyPlacement` still reparents one shell and the Preferred Model sync still finds it there. It is an icon-only `--ui-accessory-height` auto track at every width (`position: static`, no `--prompt-model-gate-width`, no 420px text-collapse); the full string lives on `aria-label` + tooltip + the shell-sibling `.prompt-model-gate-live` only. It is never overlaid on `.prompt-input`, visual card chrome must not wrap it, it is not a fifth overlay family, and it must not restore `tabindex` on that note. The gate must never grow `--topbar-height`, switch the shell or `.composer` to `height: auto`, or reach up to `.app-shell` with `:has()`: the 2026-09-11 report was that the 2026-09-09 in-flow chip made the topbar jump 51 → 83px and back on every bootstrapping / applying / failed cycle, and the later float (`position: absolute; top: calc(100% + var(--space-1)); right: 0`) covered the tab row. Keep the 51px bar; do not resurrect that float, the `:has()` grow, or `--prompt-model-gate-reserve`. The edit-mode preview (`.topbar-edit-slot-composer`) hides it because that livebar is an `overflow-x: auto` scroll container. The leftover page caret owner (`mode: page`) still stamps those same `chatclub_page_caret_*` seeds (not `chatclub_focus_guard_*`) so the child `document_start` shield is born with the page-caret lease already armed; consume reads those seeds and does not strip them until release. Parent navigation awaits that prepare ACK before assigning iframe src, and re-pins the prompt browsing context on adopt ACK and on a child stolen `postMessage`. Do not resurrect `pageCaretHoldActive`.

Prompt Library is a composer-anchored `viewerModal`: keep `viewerModal`, clamp `--prompt-library-top` from the live `.prompt-shell`, and consume `--overlay-width-wide` / `--overlay-panel-offset`. Do not copy that geometry to other viewers.
...
- `--focus-ring` for `:focus-visible` on buttons, `.tab`, `.settings-tab` / `.settings-inner-tab`, folder-toggle, Pocket/History sidebar items, and `.share-option`; `--drop-indicator` for insert carets (do not reintroduce `#0a84ff`)
- Shell: `--topbar-height: 51px` (6px padding + 38px pill + 6px padding + 1px border; the 34px `.top-icon-action` controls were sized for this bar); `--prompt-collapsed-height: 40px` (center overlay; topbar slot 38px); `--composer-width` (one width for the topbar slot, the center host, and the edit slot; Overlay Chrome Contract owns its rules); `--workspace-z-sidebar` / `--workspace-z-topbar` / `--workspace-z-topbar-edit` / `--workspace-z-topbar-controls`
- Control: `--disabled-opacity: 0.48`; `--target-min: 24px` for WCAG 2.5.8 pointer targets. Inputs, chips, and list rows use `--ui-radius`, not 7px or a literal `8px`. Favicons and Settings site logos (20–24px) use `--ui-radius-xs` (4). Site-representing lists share `app/favicon/service.js` plus `ui/favicon.js`; do not add a second logo system or packaged `icons/sites/`. Tab top corners, tab-add, and code chips use `--ui-radius-tab` (5). Nested chrome (popover items, topbar-edit-slot, preview tab, summary-source-icon) uses `--ui-radius-nested` (6). Do not raise those to 8. Pills, badges, and chips use `--ui-radius-pill` (999). Do not write literal `999px`. Do not change `50%` circles or 1–3px indicators. Compact tools use `--ui-compact-height` (32). Persistent-panel chrome rows (Summary tabs/actions and Summary/Share headers) use `--ui-chrome-height` (34). Do not collapse topbar 38 into either scale, and do not raise 28px accessories to 32. Those 28px accessories consume `--ui-accessory-height`. Isolated author-styled checkbox wrappers (`.settings-check`, `.tooltip-toggle-switch`, `.shortcut-toggle`, `.overlay-confirm-ack-box`) expand to `--target-min` without growing the visual box. Settings-pane boolean glyphs (`.appearance-toggle-control input`, `.model-preference-secondary-toggle input`, `.settings-check input`, `.tooltip-toggle-switch input`) stay 19px; `--target-min` expands wrappers only, never `min-width`/`min-height` on the native checkbox input. Preference-row booleans (`.appearance-toggle-control`, `.model-preference-secondary-toggle`) hug copy with no island well. Appearance General is a two-column split: Theme/Language/Columns on the left, Pocket icon and click-reorder on the right; those columns share Theme|Pocket, Language|click-reorder rows via subgrid. General click-reorder keeps the switch to the right of the copy and hugs that copy; the help and checkbox share the 36px `--settings-control-height` row used by the left selects; do not stretch the checkbox to the far edge of the well, and do not pin those controls to a far-right title|control rail or a 32rem stacked column. Appearance Overlays stays a single column inside the existing settingsBlock: Loading, then a hairline, then Model. Both rows use the same grammar (`.appearance-toggle-control` switch | title + ghost info trigger | compact slider) so checkboxes, titles, and slider tracks each share a left-aligned subgrid column; do not omit the Loading switch, do not let different title widths stagger the text, and do not pin `%` or the switch to a far-right rail. Overlay field help folds into the existing segmented-info ghost trigger beside the title (`border: 0`, `--target-min`, `--ui-radius`), not a ringed `icon-button` and not a second line of copy; range rows still hug a compact `220px` slider and `48px` value; Settings `.appearance-range-control` defaults to those two hugging columns with no `1fr` help column. Appearance Topbar Input font-size uses that same hug-row without a switch (title + ghost info | compact slider); do not stack it with `field()` or `.appearance-range-help`. Appearance Topbar Input placement is a native `select` (`topbar` | `center`, default `topbar`) in the same Input tab above that hug-row. Pocket/History keep `pocket-size-*`. Do not stretch those sliders across the 820px well, and do not reuse `.preference-row` / `.model-preference-row` / `is-rail-break`
- Control states: `--control-hover` → `--hover` and must be consumed by Settings tab / inner-tab / list-row hover, Tabs sidebar item/folder hover, compact-icon / top-icon-action / popover-button hover, and Pocket/History item hover (not declared-only). `--control-pressed` and `--control-selected` for press and selected fills, including official-rules mode pressed. Segmented checked stays a denser primary mix; do not invent `--control-selected-strong`. Composer `.prompt-mode-chip[aria-pressed="true"]` is filled `--primary` / `--on-primary` like Send and pin-pressed (hover must not revert it to `--control-hover` / `--text`); unpressed composer accessories (mode chip, plus, clear) rest `--text` on transparent like `.top-icon-action` and must not rest on `--muted` or fill plus/clear `--primary`; other segmented checked stays `--control-selected`. Send hover mixes `--on-primary`, not `#ffffff`. Isolated danger deletes use a 13% `--danger` fill. `--toast-text` follows `--text`
- Type: `--font-family` is system-ui. Do not load Inter. `--font-size` 13px / `--font-size-sm` 12px / `--font-size-xs` 11px / `--font-size-md` 15px / `--font-size-display` 18px for modal and persistent-panel titles. Page 15px titles consume `--font-size-md`. Do not reintroduce heading `17px`. 11/12/13 consume `--font-size-xs` / `--font-size-sm` / `--font-size`; do not mass-replace 14px or lift it to 15. Spacing `--space-1`…`--space-4` is the 4px scale; padding/margin/gap of 4/8/12/16 consume those tokens. Do not invent tokens for 6/10/14. Weight is `--font-weight-normal` 500 / `--font-weight-medium` 600 / `--font-weight-semibold` 650 / `--font-weight-bold` 760 / `--font-weight-heavy` 800 — do not reintroduce 560–780 outliers
- Drag-to-reorder surfaces (Tabs sidebar, App picker, layout presets, Settings HTML5 lists) keep drag as an enhancement and expose clickable Move up / Move down (WCAG 2.5.7). Do not make dragging the only path. Do not add those actions to `TABS_SIDEBAR_HOVER_BUTTONS`. Settings list first columns consume `--ui-reorder-cluster`. Hidden click-reorder is the default (`settingsClickReorderButtonsEnabled` false): shrink that track to the grip and hide Move up / Move down completely on Settings lists, the App picker popover, and layout presets. Do not reveal those buttons on `.settings-reorder:focus-within` or click-path, never on whole-row `:focus-within`. Turning the Appearance switch on is the click alternative. Settings buttons stay in-flow inside the cluster and must not overlay the site-mark track (`--settings-site-mark`). Platform identity uses three leading tracks: reorder cluster, site mark, name. The Appearance switch only makes those click-reorder buttons always-visible. Built-in and custom platform lists pin the Image Paste Strategy column to `--settings-image-strategy-track` (120px desktop, 104px at `max-width: 1040px`) so the icon cells stay tight while that header wraps to two lines. Do not size that track with `max-content` or a `.fr` fraction
- Search fields share one `:focus-within` ring (`.shortcut-search`). Official-rules tabs consume the inner-tabs track (`calc(var(--ui-radius) + 2px)`) and `--control-selected`
...
- Call `preserveCurrentWorkspaceForNewChat` before `startNewChatInFrame`. Preserve when the workspace still has a conversation: caller hrefs, the live snapshot, or the last durable snapshot. Skip only when that whole workspace is already a home/root New Tab. A Notion `/ai` (or other home) active frame must not skip preserve while Grok, Kagi `/c/{id}`, or another pane still has a conversation. Persist the retained conversation snapshot under the current workspace id, not a home-only live capture.
...
- The rebound id must never receive the frozen conversation. The 2026-09-07 failure was that the post-adopt persist captured the still-live pre-navigation DOM, so the new id started with the old thread, and conversation retention (`snapshotWithRetainedConversation` in the page store `save()` and in background `persistWorkspaceSessionSnapshot`) then treated every later home capture as transient. After a browser restart the "New Tab" row reopened the frozen conversation. Callers pass every frame they are about to reset (the iframe element; `{ instanceId, href }` records and plain hrefs are also accepted); `preserveCurrentWorkspaceForNewChat` marks those frames with `markFrameNewChatPending` (`iframe.dataset.newChatPending`, `app/workspace/frame-loading.js`) synchronously after `adopt()` and before its first persist, and `startNewChatInFrame` marks its frame before its first await. While marked, `currentHrefForWorkspaceTab` reports the frame at its app home instead of `preferredWorkspaceTabHref`, because the iframe `src` attribute keeps the last assigned URL until the new navigation commits and `currentThreadHref` / late old-document reports can still name the thread. `completeFrameLoading` clears the marker and recaptures only when the finished load is `new-topic`; `beginFrameLoading` clears it when any non-`new-topic` ChatClub navigation (including `restoring`) supersedes the reset, because the frame is then heading somewhere else. Do not clear it on an arbitrary location report, do not treat a `restoring` load completion as the New Chat home load, and do not disable retention to work around this.
- The background must never read the page's current `#workspace=` hash from `MessageSender.url`. Chromium freezes `sender.url` at the URL the extension document was loaded with, and `history.replaceState` from `installWorkspaceId` / the New Chat adopt never refreshes it, while `sender.tab.url` and `tabs.query` do follow the same-document navigation (measured in Chromium 2026-09-07; the earlier "lagged URL" notes below described this frozen sender URL, not a lagging tab URL). The second-New-Chat failure was that every persist of the first rebound desk was rejected as "does not match the page URL" against the frozen load-time hash, so that desk never became durable and the next New Chat replaced it in place while the sidebar still looked right. `senderWorkspaceContext` (`background/workspace-session-helpers.js`) reads the hash from the tab record first, keeps the load-time sender URL only as a fallback candidate, and accepts the requested workspace when any browser-attested URL of that tab names it; persist, claim (including the opening-claim token), and commit must all go through it. `sender.url` stays valid only for extension-origin checks.
- Background persist of the rebound workspace must detach the previous workspace as a remembered Tabs row, not as crash recovery. `tabs.onUpdated` normally observes the New Chat `replaceState` before that persist reaches the worker, so `registerWorkspaceSessionTab` must perform the same detach when it rebinds the tab from one workspace to another (`shouldDetachReplacedWorkspaceBinding`; a rememberable desk becomes a detached remembered row, never a crash-recovery candidate); otherwise the old record stays owned by the live tab and the later persist sees no binding change to detach. A URL record that still shows the old hash must not block that detach.
- After that detach, listing, claim, and title updates must treat the live tab as the rebound workspace when the durable binding already moved, even if `tab.url` still shows the old hash. The frozen row must stay remembered (`live: false`), not appear as the current live tab. Opening it must restore that snapshot instead of forking an empty desk.
- Extension reload must not resurrect that detached hash. If this tab's binding is already the rebound workspace, claim that binding instead of the lagged URL. `tabs.onUpdated` / `registerWorkspaceSessionTab` must not reattach that detached hash onto this tab. The page may already carry the rebound workspace id from `replaceState` while the extension message sender still shows the frozen load-time hash; claim, persist, and commit must succeed instead of failing closed as a URL mismatch, and that must not depend on the previous workspace already being detached (`reboundWorkspaceIdForStaleUrl` is a fallback for a detached stale hash, not the primary path). A rebound desk with a real topic title must stay in ChatClub Tabs even when iframe hrefs are still home/root, because History can record the chat before a conversation URL exists.
...
- Focusing a live ChatClub tab that lives in another browser window must move it into the requesting ChatClub window, then activate it. `windows.update` on a Little Arc or other-space window is not a Fun-window open.

Acceptance: `tools/workspace-new-chat-preserve-test.cjs`.

## Desk Auto-Naming

A desk's `topicTitle` normally comes from the first composer prompt. Conversations opened inside the frames (site sidebar, Pocket restore, History, a chat picked after New Chat) never pass through the composer, so the 2026-09-07 failure was a preserved desk labelled after its Pocket layout ("Restore group") while the live one said "New Tab". Two rules fix that; do not add a third label source at a call site.

- `layoutName` describes how a desk was produced, never what it holds. The Tabs sidebar `GENERIC_WORKSPACE_TAB_NAME` treats `ChatClub`, `Prompt`, and `pocket.restoreBatch` in every shipped locale (`Restore group`, `一键恢复`) as generic; an untitled row falls back to its chat apps, then to `ChatClub N`. When adding a temporary layout name or a new locale for that key, extend the regex.
- `app/workspace/auto-title-controller.js` names an untitled desk from its frames. `runtime.js` calls `observeFrame` on every `location` lifecycle event and every finished `loading` event. A frame is eligible only when `conversationHrefFromLocation(dataset.currentHref || dataset.currentThreadHref)` names a conversation, the frame is not `newChatPending`, and `topicTitle.canAutoGenerate()` holds. Probes run on the bounded back-off schedule `900 / 2600 / 6000 / 12000 ms` because SPAs publish titles and render turns after the route commits; every attempt re-checks the same workspace id, the same conversation href, and that no composer/Pocket generation is in flight (`topicTitle.isGenerating()`, which defers the attempt instead of racing that request). The schedule stops on the first name, on New Chat, when the frame leaves the conversation, or when the page is rebound to another workspace id.
- Source order per attempt: `getPageMeta` document title through `conversationTitleFromDocumentTitle` (`shared/topic-title.js`), which strips the site brand (built-in brands, the caller's app name, hostname labels) and placeholder titles (`New chat`, `新对话`, `Just a moment…`, sign-in pages) and yields `""` for a bare brand; then the read-only Message Navigator probe `getConversationOpening` (`content-src/message-navigator/engine.js` `conversationOpening`, `features: ["message-navigator"]`), which collects with the frame's navigator site config without enabling the navigator UI and returns the first user turn, which then goes through `maybeGenerateFromPrompt` like a composer prompt. A published title is adopted through `maybeAdoptTitle`, never generated. A probe whose reported href names a different conversation is discarded. Sites without a navigator config only get the document-title path.
- Pocket batch restore also seeds `maybeGenerateFromPrompt` with the batch's first `userMessage`; the frame probe still runs but defers while that generation is in flight.
- Custom titles and existing auto titles are never overwritten. Do not name a desk after its app, hostname, or the ChatClub document title; `itemLabel` already owns those fallbacks.
