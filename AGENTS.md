...
- Control: `--disabled-opacity: 0.48`; `--target-min: 24px` for WCAG 2.5.8 pointer targets. Inputs, chips, and list rows use `--ui-radius`, not 7px or a literal `8px`. Compact tools use `--ui-compact-height` (32). Persistent-panel chrome rows (Summary tabs/actions and Summary/Share headers) use `--ui-chrome-height` (34). Do not collapse topbar 38 into either scale, and do not raise 28px accessories to 32
- Control states: `--control-hover` → `--hover` and must be consumed by Settings tab / inner-tab / list-row hover (not declared-only). `--control-pressed` and `--control-selected` for press and selected fills. Segmented checked stays a denser primary mix; do not invent `--control-selected-strong`. `--toast-text` follows `--text`
- Type: `--font-family` is system-ui. Do not load Inter. `--font-size` 13px / `--font-size-sm` 12px / `--font-size-xs` 11px / `--font-size-md` 15px / `--font-size-display` 18px for modal and persistent-panel titles. Do not reintroduce heading `17px`. Spacing `--space-1`…`--space-4` is the 4px scale. Weight is `--font-weight-normal` 500 / `--font-weight-medium` 600 / `--font-weight-semibold` 650 / `--font-weight-bold` 760 / `--font-weight-heavy` 800 — do not reintroduce 560–780 outliers
