...
Typed `modal()` must stay a `div`/`section` overlay: `role="dialog"` (or `alertdialog` for `confirmationModal`), `aria-modal="true"`, `aria-labelledby` on the title, `aria-describedby` on confirmation body copy, scroll lock while open, Tab containment, and focus return on close. While any typed modal is open, `document.body` siblings except the topmost `.modal-backdrop`, `.toast-host`, and `#chatclub-global-tooltip` / `.global-tooltip` receive `inert`. Nested backdrops stay inert so only the top dialog is interactive. Do not use `aria-hidden` on a focused ancestor. Confirmation initial focus lands on the least-destructive footer action. Confirmation chrome may tint the title with `--danger` via `.modal-alertdialog` and may place the existing `alert` icon beside that title; do not invent a new overlay family for that. Do not add Escape-to-close here — Overlay Dismissal Policy owns that. Nested typed modals stack with `--overlay-z-modal-nested`; the nested backdrop stays transparent so dimming does not compound. Nested modals stay below popovers.
