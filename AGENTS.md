...
- Simple destructive confirms that only need a title, body, and footer actions may call `openConfirmationAction`. That helper is not a fifth overlay family: it always opens `confirmationModal` and accepts `tone` (`danger` | `warning` | `neutral`, default `danger`) plus the existing button `variant`. Official-rules mutations use this helper instead of a private confirmation wrapper. Do not use `window.confirm` or native `<dialog>`.
...
Typed `modal()` must stay a `div`/`section` overlay: `role="dialog"` (or `alertdialog` for `confirmationModal`), `aria-modal="true"`, `aria-labelledby` on the title, `aria-describedby` on confirmation body copy, scroll lock while open, Tab containment, and focus return on close. While any typed modal is open, `document.body` siblings except the topmost `.modal-backdrop`, `.toast-host`, and `#chatclub-global-tooltip` / `.global-tooltip` receive `inert`. Nested backdrops stay inert so only the top dialog is interactive. Do not use `aria-hidden` on a focused ancestor. Confirmation initial focus lands on the least-destructive footer action. Confirmation chrome uses `tone` `danger` | `warning` | `neutral` (default `danger`) via `data-overlay-tone` on `.modal-alertdialog`. Danger and warning may tint the title with `--danger` / `--warning` via `.modal-alertdialog` and may place the existing `alert` icon beside that title; neutral keeps the title in `--text` and omits the icon. Confirmation surfaces consume `--overlay-width-compact`. Confirmation footer `.button-danger` may fill `--danger` with `--on-primary` label; hover and active may darken that fill. `.overlay-confirmation` is the helper body skin, not a fifth overlay family. `.overlay-warning-card` is the shared warning-card skin; `.io-sensitive-warning` is an alias, not a second card system. The header close control stays and is equivalent to Cancel. Do not invent a new overlay family for that. Do not add Escape-to-close here — Overlay Dismissal Policy owns that. Nested typed modals stack with `--overlay-z-modal-nested`; the nested backdrop stays transparent so dimming does not compound. Nested modals stay below popovers.
