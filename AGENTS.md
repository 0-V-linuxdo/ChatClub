...
- Search fields share one `:focus-within` ring (`.workspace-tabs-sidebar-search`, `.shortcut-search`). ChatClub Tabs search stays collapsed as `.workspace-tabs-sidebar-search-toggle` until opened. Official-rules tabs consume the inner-tabs track (`calc(var(--ui-radius) + 2px)`) and `--control-selected`
