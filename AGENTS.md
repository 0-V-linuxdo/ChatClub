...
- Control states: `--control-hover` → `--hover` and must be consumed by Settings tab / inner-tab / list-row hover, Tabs sidebar item/folder hover, compact-icon / top-icon-action / popover-button hover, and Pocket/History item hover (not declared-only). `--control-pressed` and `--control-selected` for press and selected fills, including official-rules mode pressed. Segmented checked stays a denser primary mix; do not invent `--control-selected-strong`. Send hover mixes `--on-primary`, not `#ffffff`. Isolated danger deletes use a 13% `--danger` fill. `--toast-text` follows `--text`
