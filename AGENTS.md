...
- Drag-to-reorder surfaces (Tabs sidebar, App picker, layout presets, Settings HTML5 lists) keep drag as an enhancement and expose clickable Move up / Move down (WCAG 2.5.7). Do not make dragging the only path. Do not add those actions to `TABS_SIDEBAR_HOVER_BUTTONS`. Settings list first columns consume `--ui-reorder-cluster`. Compact click-reorder is the default (`settingsClickReorderButtonsEnabled` false): shrink that track and reveal Move up / Move down on `.settings-reorder:focus-within` and after pointer activation of the reorder cluster, never on whole-row `:focus-within`. Buttons stay in-flow inside the cluster and must not overlay the site-mark track (`--settings-site-mark`). Platform identity uses three leading tracks: reorder cluster, site mark, name. The Appearance switch only makes the cluster always-visible
