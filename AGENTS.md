...
Typed `modal()` must stay a `div`/`section` overlay: `role="dialog"` (or `alertdialog` for `confirmationModal`), `aria-modal="true"`, `aria-labelledby` on the title, `aria-describedby` on confirmation body copy, scroll lock while open, Tab containment, and focus return on close. Confirmation initial focus lands on the least-destructive footer action. Confirmation chrome may tint the title with `--danger` via `.modal-alertdialog` and may place the existing `alert` icon beside that title; do not invent a new overlay family for that. Do not add Escape-to-close here — Overlay Dismissal Policy owns that. Nested typed modals stack with `--overlay-z-modal-nested`; the nested backdrop stays transparent so dimming does not compound. Nested modals stay below popovers.

Page toasts use a document `.toast-host` plus per-item `role="status"` / `role="alert"` live regions. Appearance `toastStay` (`short` / `default` / `long`) sets the auto-dismiss window: 1.6s / 3.2s / 32s, errors double. Hover pauses auto-dismiss. Frame toasts keep the same roles and timing inside `.chat-frame-wrap`; a visible frame toast must accept pointer events so hover-pause works. Do not add a close button to status toasts. Global tooltip may set `aria-describedby` on the trigger while visible. Every `.popover-menu` carries `.overlay-surface`. Linear action menus may use `role="menu"` with `aria-haspopup="menu"` / `aria-expanded` on the trigger and `bindLinearMenuKeyboard` (Arrow / Home / End, roving tabindex). Multi-column pickers are not menus and must not claim `role="menu"`.
