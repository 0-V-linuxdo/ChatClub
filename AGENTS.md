...
- Control: `--disabled-opacity: 0.48`; `--target-min: 24px` for WCAG 2.5.8 pointer targets. Inputs, chips, and list rows use `--ui-radius`, not 7px or a literal `8px`. Favicons use `--ui-radius-xs` (4). Tab top corners, tab-add, and code chips use `--ui-radius-tab` (5). Nested chrome (popover items, topbar-edit-slot, preview tab, summary-source-icon) uses `--ui-radius-nested` (6). Do not raise those to 8. Compact tools use `--ui-compact-height` (32). Persistent-panel chrome rows (Summary tabs/actions and Summary/Share headers) use `--ui-chrome-height` (34). Do not collapse topbar 38 into either scale, and do not raise 28px accessories to 32. Isolated author-styled checkbox wrappers (`.settings-check`, `.tooltip-toggle-switch`, `.shortcut-toggle`) expand to `--target-min` without growing the visual box
...
- Type: `--font-family` is system-ui. Do not load Inter. `--font-size` 13px / `--font-size-sm` 12px / `--font-size-xs` 11px / `--font-size-md` 15px / `--font-size-display` 18px for modal and persistent-panel titles. Page 15px titles consume `--font-size-md`. Do not reintroduce heading `17px`. Spacing `--space-1`…`--space-4` is the 4px scale. Weight is `--font-weight-normal` 500 / `--font-weight-medium` 600 / `--font-weight-semibold` 650 / `--font-weight-bold` 760 / `--font-weight-heavy` 800 — do not reintroduce 560–780 outliers
