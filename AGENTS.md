...
- Simple destructive confirms that only need a title, body, and footer actions may call `openConfirmationAction`. That helper is not a fifth overlay family: it always opens `confirmationModal` and accepts `tone` (`danger` | `warning` | `neutral`, default `danger`), button `variant`, optional `acknowledge` copy (danger action stays disabled until checked), and optional `busyLabel`. Severity is expressed through those existing knobs, not a new family: routine item deletes use `tone="neutral"` with `variant="danger"`; risky runtime or security confirms use `tone="warning"` plus `.overlay-warning-card`; critical or batch irreversible confirms keep default `tone="danger"` and may pass `acknowledge`. Official-rules mutations use this helper instead of a private confirmation wrapper. Do not use `window.confirm` or native `<dialog>`.
