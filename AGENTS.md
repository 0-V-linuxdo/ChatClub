...
- Drag-to-reorder surfaces (Tabs sidebar, App picker, layout presets, Settings HTML5 lists) keep drag as an enhancement and expose clickable Move up / Move down (WCAG 2.5.7). Do not make dragging the only path. Do not add those actions to `TABS_SIDEBAR_HOVER_BUTTONS`. Settings list first columns consume `--ui-reorder-cluster`
