...
assert.doesNotMatch(css, /--prompt-collapsed-height:\s*56px;/);
assert.match(css, /\.composer \{\s*display:\s*block;[\s\S]*?height:\s*var\(--prompt-collapsed-height\)/);
assert.doesNotMatch(css, /\.composer \{\s*display:\s*block;[\s\S]*?height:\s*56px/);
