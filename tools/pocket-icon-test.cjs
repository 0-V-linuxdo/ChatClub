...
(async () => {
  ...
  assert.equal(topbar.pocketChromeLabelKey(), "topbar.pocket");
  assert.equal(topbar.pocketChromeLabelKey({}), "topbar.pocket");
  assert.equal(topbar.pocketChromeLabelKey({ pocketIcon: "star" }), "topbar.pocket");
  assert.equal(topbar.pocketChromeLabelKey({ pocketIcon: "pocket" }), "topbar.pocketBrand");
  assert.equal(topbar.topbarItemLabelKey({ type: "item", id: "pocket" }), "topbar.pocket");
  assert.equal(topbar.topbarItemLabelKey({ type: "item", id: "pocket" }, { pocketIcon: "pocket" }), "topbar.pocketBrand");
  ...
  assert.match(appearanceSource, /class: "field appearance-pocket-icon-field"/);
  ...
  assert.equal(i18n.t("topbar.pocketBrand"), "Pocket");
  ...
  i18n.setLanguage("en");
  assert.equal(i18n.t("topbar.pocket"), "Favorites");
  assert.equal(i18n.t("topbar.pocketBrand"), "Pocket");
