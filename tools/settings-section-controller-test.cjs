...
(async () => {
  ...
  assert.match(stylesSource, /\.api-profile-models-field \{[^}]*border-left:\s*1px solid var\(--line\);/s);
  ...
  assert.match(stylesSource, /\.api-profile-editor-layout \{[^}]*grid-template-columns:\s*minmax\(0, 2fr\) minmax\(0, 1fr\);/s);
