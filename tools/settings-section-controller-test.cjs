...
(async () => {
  ...
  assert.match(profilesSource, /t\("profiles\.provider"\)/);
  assert.doesNotMatch(profilesSource, /row\.profileName/);
  ...
  assert.match(profilesSource, /"api-profile-model-more"/);
  assert.match(profilesSource, /api-profile-model-row-solo/);
  assert.match(profilesSource, /profileModelCell/);
  assert.match(profilesSource, /api-profile-models-cell is-multiple/);
  assert.match(profilesSource, /apiProfileFavoriteModels/);
  assert.match(profilesSource, /svgIcon\("shield"\)/);
  assert.match(profilesSource, /"moreVertical"/);
  assert.match(profilesSource, /class: "popover-menu overlay-surface api-profile-model-menu"/);
  assert.match(profilesSource, /starred \? "star" : "starOff"/);
  assert.match(profilesSource, /saveButton\.disabled = !valid \|\| !dirty/);
  assert.match(profilesSource, /live\(\) \{/);
  assert.match(profilesSource, /class: "api-profile-secret"/);
  assert.match(profilesSource, /class: "api-profile-editor-identity"/);
  assert.match(profilesSource, /querySelector\("\.modal-header h2"\)\?\.after/);
  assert.match(stylesSource, /\.api-profile-editor-identity-name \{[^}]*border-radius:\s*var\(--ui-radius-pill\);/s);
  assert.match(stylesSource, /\.api-profile-editor-layout::before \{[^}]*background:\s*var\(--line\);/s);
  assert.match(profilesSource, /t\("profiles\.defaultModelHint"\)/);
  assert.match(profilesSource, /svgIcon\(visible \? "eyeOff" : "eye"\)/);
  assert.match(stylesSource, /\.api-profile-model-add \{[^}]*border:\s*0;/s);
  assert.match(stylesSource, /\.api-profile-editor-layout \{[^}]*grid-template-columns:\s*minmax\(0, 1fr\) minmax\(0, 1fr\);/s);
  assert.match(stylesSource, /\.settings-editor-modal\.api-profile-editor-modal \{[^}]*width:\s*min\(920px/s);
