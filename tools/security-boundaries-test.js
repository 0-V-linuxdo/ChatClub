...
(async () => {
  ...
  const secureCommandsMatch = background.match(/const secureFrameCommands = new Set\(\[([\s\S]*?)\]\);/);
  assert.ok(secureCommandsMatch, "secure frame command allowlist must exist");
  assert.match(secureCommandsMatch[1], /"getSummaryRuntimeState"/);
