...
assert.match(smoke, /"Content-Security-Policy": "frame-ancestors 'none'"/);
assert.match(smoke, /"Content-Security-Policy-Report-Only": "frame-ancestors 'none'"/);
assert.match(smoke, /tabScopedSelfNavigation/);
assert.match(smoke, /location\.replace\(target\.href\)/);
