...
(async () => {
  ...
  assert.match(read("app/favicon/service.js"), /chatclub\.faviconCache\.v8/);
  assert.match(read("app/favicon/service.js"), /rememberDecoded/);
  assert.match(iconEditor, /function bindDiscoveredFavicon/);
  assert.match(iconEditor, /function iconImage[\s\S]*bindDiscoveredFavicon\(image/);
  assert.match(css, /data-favicon-remote/);
  ...
  assert.match(profilesSettings, /settings-icon-editor-layout/);
  assert.match(profilesSettings, /settings-icon-editor-preview/);
  ...
  assert.doesNotMatch(profilesSettings, /settings-icon-source-row/);
  assert.doesNotMatch(profilesSettings, /settings-icon-field-actions/);
  ...
  assert.match(css, /\.settings-icon-editor-layout \{/);
  ...
  assert.doesNotMatch(css, /\.settings-icon-advanced-body \{/);
  assert.doesNotMatch(css, /\.settings-icon-source-row \{/);
  assert.doesNotMatch(css, /\.settings-icon-field-actions \{/);
  assert.match(css, /\.api-profile-models-field \{[^}]*align-content:\s*start/s);
  assert.match(css, /\.settings-icon-editor-tools \{[^}]*display:\s*flex/s);
  assert.doesNotMatch(css, /\.settings-icon-editor-tools \{[^}]*justify-content:\s*flex-end/);
