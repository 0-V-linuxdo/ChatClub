...
(async () => {
  ...
  assert.match(panelSource, /claimOverlaySearchCaret\(field, searchCaretOptions\(\)\)/);
  assert.match(panelSource, /pinOverlaySearchCaret\(\)/);
  assert.match(panelSource, /releaseOverlaySearchCaret/);
  assert.doesNotMatch(panelSource, /restoreSearchFieldAfterFrameLoad/);
  assert.doesNotMatch(panelSource, /FRAME_LOAD_SEARCH_FOCUS/);
  assert.doesNotMatch(panelSource, /addEventListener\("load"/);
