...
import { claimWorkspaceSessionRecovery, closeOtherLiveWorkspaceTabs, commitWorkspaceSessionRecovery, detachWorkspaceSessionMirror, dismissClearedWorkspaceTabs, exportRememberedWorkspaceTabs, focusWorkspaceTab, forgetRememberedWorkspaceTab, handleWorkspaceSessionAlarm, importRememberedWorkspaceTabs, listClearedWorkspaceTabs, listLiveWorkspaceTabs, persistWorkspaceSessionSnapshot, prepareWorkspaceSessionLifecycle, registerWorkspaceSessionTab, restoreClearedWorkspaceTabs, rotateWorkspaceSessionGeneration, setWorkspaceTabTitle } from "./workspace-session.js";
...
const backgroundRequestHandlers = [
  ...
  [REQUEST.LIST_LIVE_WORKSPACE_TABS, async (_message, sender) => { await prepareWorkspaceSessionLifecycle(chrome, { reason: "list-remembered-workspace-tabs" }); return listLiveWorkspaceTabs(chrome, {}, sender); }], [REQUEST.EXPORT_REMEMBERED_WORKSPACE_TABS, async () => { await prepareWorkspaceSessionLifecycle(chrome, { reason: "export-remembered-workspace-tabs" }); return exportRememberedWorkspaceTabs(chrome); }],
  [REQUEST.IMPORT_REMEMBERED_WORKSPACE_TABS, async (message) => { await prepareWorkspaceSessionLifecycle(chrome, { reason: "import-remembered-workspace-tabs" }); return importRememberedWorkspaceTabs(chrome, message); }], [REQUEST.FOCUS_WORKSPACE_TAB, (message, sender) => focusWorkspaceTab(chrome, message, sender)], [REQUEST.SET_WORKSPACE_TAB_TITLE, (message) => setWorkspaceTabTitle(chrome, message)], [REQUEST.FORGET_REMEMBERED_WORKSPACE_TAB, (message, sender) => forgetRememberedWorkspaceTab(chrome, message, { sender })], [REQUEST.CLOSE_OTHER_LIVE_WORKSPACE_TABS, (message, sender) => closeOtherLiveWorkspaceTabs(chrome, message, sender)], [REQUEST.RESTORE_CLEARED_WORKSPACE_TABS, (message, sender) => restoreClearedWorkspaceTabs(chrome, message, sender)],
