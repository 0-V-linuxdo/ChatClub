(() => {
  ...
  var FRAME_COMMAND_SPECS = Object.freeze({
    ...
    sendText: command({ timeoutMs: 12e3, mutating: true }),
    ...
    deleteThread: command({ timeoutMs: 37e3, mutating: true }),
    getDeleteConfirmState: command({ timeoutMs: 2400 }),
    applyPreferredModel: command({ timeoutMs: 18e3, mutating: true }),
    cancelPreferredModelApply: command({ timeoutMs: 2e3, mutating: true }),
    setMessageNavigator: command({ timeoutMs: 6e3, mutating: true }),
    hideMessageNavigatorMenu: command({ timeoutMs: 2e3, mutating: true }),
    getMessageNavigatorState: command({ timeoutMs: 2e3 })
  });

  // shared/topbar.js
  var TOPBAR_BUILTIN_ITEMS = [
    "brand",
    "settings",
    "composer",
    "newChat",
  ...
  var TOPBAR_SETTINGS_SECTION_ITEMS = {
  ...
  var TOPBAR_SETTINGS_ITEM_SECTIONS = Object.fromEntries(
    Object.entries(TOPBAR_SETTINGS_SECTION_ITEMS).map(([section, itemId]) => [itemId, section])
  );
  var TOPBAR_SETTINGS_SECTION_ITEM_IDS = Object.values(TOPBAR_SETTINGS_SECTION_ITEMS);
  var TOPBAR_REQUIRED_ITEMS = ["settings", "composer", "settingsJumpMenu"];
  var DEFAULT_TOPBAR_LAYOUT = [
    { type: "item", id: "brand" },
  ...
  var BUILTIN_SET = new Set(TOPBAR_BUILTIN_ITEMS);
  var REQUIRED_SET = new Set(TOPBAR_REQUIRED_ITEMS);

  // shared/constants.js
  var API_PROFILE_ENDPOINT_DEFAULT = "https://api.openai.com/v1/chat/completions";
  var API_PROFILE_MODEL_DEFAULT = "gpt-3.5-turbo";
  var API_PROMOTION_CHANNELS_VERSION = 2;
  var API_PROFILE_ZERO_ZERO_ENDPOINT = "https://api.0-0.pro/v1/chat/completions";
  var API_PROFILE_ZERO_ZERO_MODEL = "gpt-5.5";
  var API_PROFILE_ZERO_ZERO_REGISTER_URL = "https://0-0.pro/register?ref=CSLPRL76";
  var SCRIPT_CONFIG_SCHEMA_VERSION = 3;
  var PROMPT_IMAGE_PASTE_STRATEGY_SEQUENTIAL = "sequential";
  var PROMPT_IMAGE_PASTE_STRATEGY_BATCH = "batch";
  var PROMPT_IMAGE_PASTE_STRATEGIES = Object.freeze([
    PROMPT_IMAGE_PASTE_STRATEGY_SEQUENTIAL,
    PROMPT_IMAGE_PASTE_STRATEGY_BATCH
  ]);
  var DEFAULT_PROMOTION_API_PROFILES = [
    {
  ...
  var GEMINI_THINKING_LEVEL_PREFERENCE_KEY = "GeminiThinkingLevel";
  var GEMINI_THINKING_LEVEL_TARGETS = Object.freeze([
    Object.freeze({ id: "standard", label: "Standard" }),
    Object.freeze({ id: "extended", label: "Extended" })
  ]);
  var DEFAULT_GEMINI_THINKING_LEVEL = "standard";
  var MODEL_PREFERENCE_TARGETS = Object.freeze({
  ...
  var DEFAULT_MODEL_PREFERENCES = Object.freeze(
    {
  ...
  var DEFAULT_MODEL_PREFERENCE_ORDER = Object.freeze(Object.keys(MODEL_PREFERENCE_TARGETS));
  var TAB_GROUP_HEADER_BUTTONS = [
  ...
  var DEFAULT_TAB_GROUP_BUTTON_PLACEMENT = Object.freeze(
    Object.fromEntries(TAB_GROUP_HEADER_BUTTONS.map((item) => [item.id, item.defaultPlacement || "pinned"]))
  );
  var DEFAULT_TAB_GROUP_BUTTON_ORDER = Object.freeze(
    TAB_GROUP_HEADER_BUTTONS.filter((item) => !item.requiredPinned).map((item) => item.id)
  );
  var DEFAULT_POCKET_CARD_SIZE = Object.freeze({
    width: 460,
    height: 560
  });
  var TOOLTIP_TARGET_GROUPS = Object.freeze([
    Object.freeze({
      id: "topbar",
  ...
  var TOOLTIP_TARGET_IDS = Object.freeze(
    TOOLTIP_TARGET_GROUPS.flatMap((group) => group.targets.map((target) => target.id))
  );
  var OPTIMIZE_PROMPT_TEMPLATE_DEFAULT = `You are an AI prompt expert, skilled at analyzing and optimizing user-provided prompts.
...
- Return only the optimized prompt.`;
  var SUMMARY_PANEL_PROMPT_DEFAULT = `Summarize the selected chat context. Keep the answer concise, factual, and useful.
...
When the user asks a follow-up question, answer from the provided context first and clearly say when the context is insufficient.`;
  var DEFAULT_FRAME_TOAST_POSITION = Object.freeze({ x: 100, y: 100 });
  var DEFAULT_OPTIONS = {
  ...
  var FIXED_KEY_ACTIONS = ALL_SHORTCUT_ACTIONS.filter((action) => !PATTERN_ACTIONS.includes(action));
...
${value}`);
  ...
  function userscriptLooksLikeCopyIcon(el) {
    const signature = svgSignature2(el);
  ...
  function installContentBridge() {
    ...
    const {
      ...
      cleanCaptured: cleanCaptured2,
      pageMeta: pageMeta2,
      copyLooksUseful: copyLooksUseful2,
      hasUserAndAssistant: hasUserAndAssistant2,
      elementOrder: elementOrder2,
      classText: classText2,
      buttonText: buttonText2,
      userscriptMeta: userscriptMeta2,
      matches: matches2,
      userscriptLooksLikeCopyIcon: userscriptLooksLikeCopyIcon2,
      internalTool: internalTool2,
      userscriptButtonOk: userscriptButtonOk2,
      userscriptCopyRoots: userscriptCopyRoots2,
      userscriptFindCopyButtons: userscriptFindCopyButtons2,
      userscriptFindMenuButtons: userscriptFindMenuButtons2,
      userscriptOpenCopyButtons: userscriptOpenCopyButtons2,
      userscriptCloseMenus: userscriptCloseMenus2,
      copyMatches: copyMatches2,
      userscriptCopyAccepted: userscriptCopyAccepted2,
      ...
      nodeTextForCopy: nodeTextForCopy2,
      nativeCopyDedup: nativeCopyDedup2,
    ...
    const COPY_SOURCE2 = PROTOCOL.NATIVE_COPY_SOURCE;
    ...
    const PREFERRED_MODEL_FOCUS_SHIELD_LEASE_MS = 5e3;
    ...
    const PROMPT_IMAGE_PASTE_STRATEGY_BATCH2 = "batch";
    let contentDocumentId = window.__CHATCLUB_CONTENT_DOCUMENT_ID__ || `${Date.now()}-${Math.random().toString(36).slice(2, 11)}`;
    ...
    function clearSubmissionNavigationForTrustedIntent(event) {
      ...
      const keyboardNavigation = event.type === "keydown" && (event.altKey && (key === "ArrowLeft" || key === "ArrowRight") || (event.metaKey || event.ctrlKey) && (key === "[" || key === "]") || (key === "Enter" || key === " ") && target);
    ...
    function currentSubmissionNavigation(kind = "") {
      ...
      const candidate = activeSubmissionNavigation || normalizedSubmissionNavigation(window.__CHATCLUB_ACTIVE_SUBMISSION_NAVIGATION__ || {});
    ...
    if (window.__CHATCLUB_CONTENT_BRIDGE_VERSION__ === CONTENT_BRIDGE_VERSION2) {
    ...
    window.__CHATCLUB_CONTENT_BRIDGE_VERSION__ = CONTENT_BRIDGE_VERSION2;
    window.__CHATCLUB_CONTENT_BRIDGE_INSTALLED__ = true;
    function contentBridgeIsCurrent() {
      return window.__CHATCLUB_CONTENT_BRIDGE_VERSION__ === CONTENT_BRIDGE_VERSION2;
    }
    try {
    ...
    function promptImagePasteStrategy(data = {}) {
      return String(data?.imagePasteStrategy || "").trim().toLowerCase() === PROMPT_IMAGE_PASTE_STRATEGY_BATCH2 ? PROMPT_IMAGE_PASTE_STRATEGY_BATCH2 : "sequential";
    }
    ...
    async function sendTextUncached(data) {
      ...
      const batch = promptImagePasteStrategy(data) === PROMPT_IMAGE_PASTE_STRATEGY_BATCH2;
    ...
    function preferredModelCancelled(context) {
      ...
      return !context || context.signal?.aborted || activePreferredModelRun !== context || !contentBridgeIsCurrent() || !tokenIsCurrent;
    }
    ...
    function visibleDeleteCandidates(root = document, selector = DELETE_CLICKABLE_SELECTOR) {
      return qsa2(selector, root, { all: true }).filter((element) => visible2(element) && !isDisabledElement(element));
    }
    ...
    function deleteConfirmActivationElementMatches(element, expected, root = null) {
      ...
      if (!candidate || !visible2(candidate) || isDisabledElement(candidate)) return false;
    ...
    function kagiDeleteMenuItem(trigger = null, labels = ["Delete", "Delete thread", "Delete chat", "Remove", "删除"]) {
      ...
      const add = (element, extraScore = 0) => {
        if (!element || seen.has(element) || !visible2(element) || isDisabledElement(element)) return;
        const value = deleteElementText(element);
        if (!deleteLabelMatchesExactish(value, labels)) return;
        if (deleteLabelMatches(value, DELETE_CANCEL_LABELS)) return;
        const target = deleteClickableElement(element);
        if (!target || seen.has(target) || !visible2(target) || isDisabledElement(target)) return;
        const rect = modelRect(target);
    ...
    function findDeleteMenuItem(root, labels) {
      ...
      if (!candidates.length) {
        for (const element of qsa2("[role='menuitem'],[role='option'],button,[role='button'],li,div,span", root, { all: true })) {
          if (!visible2(element) || isDisabledElement(element)) continue;
    ...
    function topRightMenuTrigger({ labels = [], selectors = [] } = {}) {
      ...
      for (const element of qsa2(selector, document, { all: true })) {
        const target = deleteClickableElement(element);
        if (!target || seen.has(target) || !visible2(target) || isDisabledElement(target)) continue;
        seen.add(target);
        const rect = modelRect(target);
    ...
    function geminiDeleteMenuItemMatches(node) {
      if (!node || !visible2(node) || isDisabledElement(node) || geminiDeleteMenuItemLooksLikeNotebook(node)) return false;
    ...
    function findGeminiDeleteMenuItem(trigger = null) {
      ...
      const add = (node, root, extraScore = 0) => {
        ...
        if (!target || target === root || seen.has(target) || !visible2(target) || isDisabledElement(target) || geminiDeleteMenuMarkerCount(target) > 1 || geminiDeleteMenuItemLooksLikeNotebook(target)) return;
    ...
    async function clickDeepSeekSidebarToggleByPoint() {
      ...
      for (const point of points) {
        ...
        const rect = modelRect(button);
        if (!button || !rect || !visible2(button) || isDisabledElement(button)) continue;
    ...
    function deepSeekTopicRows(root, hints = deepSeekDeleteHints()) {
      const candidates = [];
      const seen = /* @__PURE__ */ new Set();
      for (const element of qsa2("a,button,[role='button'],li,div", root || document, { all: true })) {
        if (!element || seen.has(element) || !visible2(element)) continue;
        const rect = modelRect(element);
    ...
    async function deleteDeepSeekThread(data = {}) {
      ...
      if (!moreButton) {
        ...
        return data?.trustedMenuClickRetried ? deleteResult(false, "deepseek", reason) : data?.trustedKeySequenceRetried ? deleteResultWithDeepSeekTrustedMenuClick(reason, row) : deleteResultWithDeepSeekTrustedKeySequence(reason, row);
      }
      if (!await openDeepSeekTriggerAndClickDelete(moreButton, labels, { timeoutMs: 2600, allowHiddenTrigger: true })) {
        ...
        return data?.trustedMenuClickRetried ? deleteResult(false, "deepseek", reason) : data?.trustedKeySequenceRetried ? deleteResultWithDeepSeekTrustedMenuClick(reason, row, moreButton) : deleteResultWithDeepSeekTrustedKeySequence(reason, row);
      }
      const confirmed = await clickDeleteConfirmIfPresent(6500);
    ...
    function scoreGeminiModelMenuRoot(root) {
      ...
      const text3 = modelElementText(root);
      const token = compactModelText(text3);
      if (!token) return 0;
      const keys = geminiModelKeysFromText(text3);
    ...
    function geminiModelItems(root) {
      ...
      const add = (element) => {
        ...
        if (!row || seen.has(row) || !visible2(row) || isDisabledElement(row)) return;
        if (row === root || modelElementArea(row) > rootArea * 0.92) return;
        const text3 = modelElementText(row);
        if (!compactModelText(text3)) return;
        seen.add(row);
        rows.push(row);
      };
    ...
    function geminiCompactMenuRows(root) {
      ...
      for (const element of qsa2("*", root)) {
        ...
        if (!row || seen.has(row) || row === root || !visible2(row) || isDisabledElement(row)) continue;
        const text3 = modelElementText(row);
        if (!compactModelText(text3)) continue;
        seen.add(row);
        rows.push(row);
      }
    ...
    function scoreGeminiModelItem(item, modelId) {
      const text3 = modelElementText(item);
      if (!geminiTargetMatchesText(modelId, text3)) return -1;
      const token = compactModelText(text3);
      const keys = geminiModelKeysFromText(text3);
    ...
    function findGeminiThinkingLevelOption(root, modelId) {
      ...
      const row = geminiCompactMenuRows(root).filter((row2) => {
        const text3 = modelElementText(row2);
        const token = compactModelText(text3);
        ...
        return geminiTargetMatchesText(modelId, text3);
    ...
    function findGeminiExtendedThinkingToggle(root) {
      ...
      for (const candidate of candidates) {
        ...
        if (!item || seen.has(item) || !visible2(item) || isDisabledElement(item)) continue;
        seen.add(item);
    ...
    function grokModelMenuItemRow(element, root, matchesSpec = null) {
      ...
      while (node && node.nodeType === 1 && node !== root) {
        if (!visible2(node) || isDisabledElement(node)) {
    ...
    function grokItemCandidates(root) {
      ...
      const candidates = [];
      const add = (element) => {
        if (!element || !visible2(element) || isDisabledElement(element)) return;
        ...
        if (!item || seen.has(item) || !root.contains?.(item) || !visible2(item) || isDisabledElement(item)) return;
    ...
    function grokModelItemText(item) {
      const text3 = modelElementText(item);
      return text3.split(/\n+/).map((part) => part.trim()).find(Boolean) || text3;
    }
    ...
    function directGrokModelTriggerBoost(element) {
      if (!element || !visible2(element) || isDisabledElement(element)) return 0;
    ...
    function scoreGrokModelTrigger(element, options = {}) {
      if (!element || !visible2(element) || isDisabledElement(element)) return -1;
      const rootsSelector = GROK_MODEL_MENU_ROOT_SELECTORS.join(", ");
    ...
    function scoreNotionModelTrigger(element, options = {}) {
      if (!element || !visible2(element) || !options.allowDisabled && isDisabledElement(element)) return -1;
    ...
    function scoreNotionModelItem(element, modelId) {
      if (!element || !visible2(element) || isDisabledElement(element)) return Number.NEGATIVE_INFINITY;
      const textValue = modelElementText(element);
    ...
    function findNotionModelItemPointTarget(element, root, modelId) {
      ...
      while (node && node.nodeType === 1 && node !== root) {
        if (visible2(node) && !isDisabledElement(node) && matchesSpec(node) && countNotionModelTargets(modelElementText(node)) <= 1) {
    ...
    function deepSeekModeCandidates() {
      ...
      for (const element of visibleSelectorElements(DEEPSEEK_MODE_SELECTORS)) {
        if (!element || !visible2(element) || isDisabledElement(element)) continue;
        ...
        if (!clickable || seen.has(clickable) || !visible2(clickable) || isDisabledElement(clickable)) continue;
    ...
    function findDeepSeekModeTarget(modeId) {
      ...
      const matches3 = deepSeekModeCandidates().filter((element) => deepSeekModeIdFromText(deepSeekModeCandidateText(element)) === modeId).map((element) => ({
        element,
      ...
      return matches3[0]?.element || null;
    }
    ...
    async function getSummaryRuntimeState() {
      ...
      const isolatedReady = Boolean(
        registry && typeof registry === "object" && Object.keys(registry).length && isolatedVersion === CONTENT_BRIDGE_VERSION2
      );
      const pageState = await pageSummaryRuntimeState2();
      const mainReady = Boolean(
        pageState?.ready && pageState.bridgeVersion === CONTENT_BRIDGE_VERSION2
      );
      return {
