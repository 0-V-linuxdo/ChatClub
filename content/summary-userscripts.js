(() => {
  ...
  var CONTENT_RUNTIME_SUMMARY_ISOLATED_BUNDLE_IDENTITY = /* @__PURE__ */ Object.freeze({ "outputPath": "content/summary-userscripts.js", "entryPath": "content-src/summary-userscripts.js", "sourceSha256": "7e3203b26f2ae9765a8b667cf1ecf5f5e1325de1f87a41c7a7ebc2b123b8a876", "implementationSha256": "bea79a13e4ca45f718cb54e4d79c69523393296c159f13fd5e3d6f4055d633a2", "implementationVersion": "2026.07.16.2+bundle.bea79a13e4ca45f718cb54e4d79c69523393296c159f13fd5e3d6f4055d633a2" });
  ...
  function createSummaryRunnerRegistry() {
    ...
    scripts["manus"] = async function(api) {
      ...
      const blankNewTask = (root2) => {
        const buttons = qsa("button,[role=button]", root2).filter(messageCopyControl);
      ...
      const copyTurns = async (root2) => {
        const buttons = qsa("button,[role=button]", root2).filter(messageCopyControl).sort(order).slice(0, 48);
        const out = [];
        const seenOwners = /* @__PURE__ */ new Set();
        for (const button of buttons) {
          const owner = messageOwner(button);
      ...
      const userBubbleTurns = (root2) => {
        const out = [];
        const seen = /* @__PURE__ */ new Set();
        const nodes = qsa("div,article,section,li,p,[data-event-id]", root2).filter((node) => looksUserBubble(node, root2)).sort(order).slice(0, 80);
        for (const node of nodes) {
          ...
          if (!text || text.length > 8e3 || chromeLine(text) || looksAssistantPreamble(text) || seen.has(text)) continue;
          seen.add(text);
