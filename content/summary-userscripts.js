  ...
  function createSummaryRunnerRegistry() {
    ...
    scripts["notion"] = async function(api) {
      ...
      const notionDomTextFallback = () => {
        ...
        const assistant = normalize(lines.slice(answerStart).filter((line) => line !== collected.user && !promptParts.includes(line) && !isChromeLine(line) && !isResearchLogLine(line) && !isMetaLine(line)).join("\n"));
    ...
    scripts["manus"] = async function(api) {
      ...
      const qs = (selector, scope = document) => {
        try {
          return api.qs(selector, scope || document);
        } catch (error) {
          return null;
        }
      };
      ...
      const useful = (value) => {
        const text = normalize(value);
        if (!text || text.length < 2 || text.length > 2e5) return "";
        if (/^(?:copy|copied|copy link|retry|edit|share|task completed|how was this result|复制|已复制|重试|编辑|分享|任务已完成)$/i.test(text)) return "";
        return text;
      };
      const chatRoot = () => qs("#manus-chat-box") || qs("#manus-agents-chat-view") || qs("#manus-home-page-session-content") || qs("main,[role=main]") || document.body || document.documentElement;
      const computerChrome = (node) => /manus['’]?s computer|computer is inactive|manus desktop/i.test(actionSignature(node) || "") || /manus['’]?s computer|computer is inactive/i.test(normalize(node && (node.innerText || node.textContent) || "").slice(0, 240));
      const pageChrome = (node) => Boolean(closest(node, "nav,aside,footer,form,input,textarea,select,[contenteditable=true]")) || computerChrome(node);
      ...
      const looksCopyIcon = (button) => {
        const text = svgSignature(button);
        if (!text) return false;
        return /copy|clipboard|lucide-copy|tabler-icon-copy|heroicons.*clipboard|mingcute.*copy/.test(text) && !/computer|desktop|monitor|sandbox|folder|file|skill|star|share|upgrade/.test(text);
      };
      ...
      const userAction = (control) => /(?:^|\b)(edit|pencil)(?:\b|$)|编辑/.test(actionSignature(control)) && !/credit|credits|computer|skill|connector|member/.test(actionSignature(control));
      const actionRow = (button) => {
        let node = button && button.parentElement;
        for (let i = 0; i < 5 && node && node !== chatRoot(); i += 1) {
          const companions = qsa("button,[role=button]", node).filter((item) => item !== button && canonicalControl(item) && (assistantAction(item) || userAction(item)));
      ...
      const filterUserText = (value) => useful(String(value || "").split(/\n+/).map((line) => normalize(line)).filter((line) => line && !chromeLine(line) && !looksAssistantPreamble(line)).join("\n"));
      ...
      const sessionTitleText = () => useful(normalize(document.title || "").replace(/^\s*manus\s*[|–—-]\s*/i, "").replace(/\s*[-|–—]\s*manus\b.*$/i, ""));
      ...
      const stripAssistantChrome = (value) => useful(normalize(String(value || "").replace(/this one['’]?s on manus[^\n]*/ig, " ").replace(/[^\n]*\bno credits charged\b[^\n]*/ig, " ").replace(/[^\n]*\bis free for a limited time\b[^\n]*/ig, " ")));
      ...
      const sanitizeConversation = (messages) => {
        ...
        for (const message of api.merge(messages || [])) {
          if (message.role === "user") {
            const text = filterUserText(message.text);
            if (text && !looksAssistantPreamble(text)) cleaned.push({ role: "user", text });
            continue;
          }
      ...
      const blankNewTask = (root2) => {
        const buttons = qsa("button,[role=button]", root2).filter(companionMessageCopy);
        if (buttons.length) return false;
        const text = normalize(root2 && (root2.innerText || root2.textContent) || "");
        return /ask manus|new task|message your task/i.test(text) && !/task completed|任务已完成/i.test(text) && text.length < 1200;
      };
      const scrollConversationStart = async (root2) => {
        try {
          let node = root2;
          ...
          const first = root2 && (root2.firstElementChild || root2);
      ...
      const copyTurns = async (root2, have = {}) => {
        const buttons = qsa("button,[role=button]", root2).filter(companionMessageCopy).sort(order).slice(0, 8);
        const out = [];
        const seenOwners = /* @__PURE__ */ new Set();
      ...
      const turnsFromEvents = (events) => {
        ...
        for (const event of events || []) {
          ...
          if (isUserChatEvent(event)) {
            const user = filterUserText(text);
            if (user) out.push({ role: "user", text: user });
          } else if (isAssistantChatEvent(event)) out.push({ role: "assistant", text: clipAssistant(text) });
        }
        return api.merge(out);
      };
      ...
      const pageChatTurns = (root2) => {
        ...
        const takeStore = (store) => {
          ...
          try {
            turnsFromEvents(eventsFromState(store.getState())).forEach(pushTurn);
          } catch (error) {
          }
        };
        const takeEvent = (event) => {
          ...
          } else if (isAssistantChatEvent(event)) pushTurn({ role: "assistant", text: clipAssistant(text) });
        };
        let visited = 0;
        const visitFiber = (fiber, depth) => {
          if (!fiber || depth > 90 || visited > 4e3) return;
          visited += 1;
        ...
        visitFiber(fiberFrom(root2), 0);
        if (root2 !== document.body) visitFiber(fiberFrom(document.body), 0);
        return api.merge(out);
      };
      const looksUserBubble = (node, root2) => {
        ...
        const text = filterUserText(node.innerText || node.textContent);
        if (!text) return false;
      ...
      const looksAssistantBubble = (node, root2) => {
        ...
        const text = normalize(node.innerText || node.textContent || "");
        return /task completed|任务已完成/.test(text) && (looksAssistantPreamble(text) || text.length > 40);
      };
      const bubbleTurns = (root2, role) => {
        const out = [];
        const seen = /* @__PURE__ */ new Set();
        const match = role === "user" ? looksUserBubble : looksAssistantBubble;
        const nodes = qsa("div,article,section,li,p,[data-event-id]", root2).filter((node) => match(node, root2)).sort(order).slice(0, 80);
        for (const node of nodes) {
          ...
          const text = useful(raw);
          if (!text || text.length > 8e3 || chromeLine(text) || role === "user" && looksAssistantPreamble(text)) continue;
          out.push({ role, text, node });
        }
        return out;
      };
      const orderedTurns = (...groups) => groups.flat().filter((item) => item && item.node).sort((a, b) => order(a.node, b.node)).map(({ role, text }) => ({ role, text }));
      const domFallback = (root2, copiedAssistant = "") => {
        const raw = normalize(root2 && (root2.innerText || root2.textContent) || "");
        if (!raw) return [];
        const lines = [];
        for (const rawLine of raw.split(/\n+/)) {
    ...
    Object.defineProperty(scripts, "runtimeVersion", { value: "2026.07.16.2+implementation.10dd8712e4309734223d632a6de25477028d380005b9472ee28a5a458595c133" });
    return scripts;
  }
