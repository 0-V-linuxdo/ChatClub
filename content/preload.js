  ...
  function installPreload() {
    ...
    function installDeepSeekDeleteBridge() {
      ...
      window.addEventListener("message", async (event) => {
        const message = event.data;
        if (message?.source !== DEEPSEEK_DELETE_SOURCE2 || message.type !== "request" || message.action !== "deleteThread") return;
        let result;
        try {
        ...
        try {
          window.postMessage({
            source: DEEPSEEK_DELETE_SOURCE2,
            type: "response",
            id: message.id,
            action: "deleteThread",
            site: "deepseek",
    ...
    if (framed && (host === "app.notion.com" || host.endsWith(".notion.so"))) {
      ...
      const wait = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
      const deadlineFromPayload = (payload = {}, fallbackMs = 1e4) => {
      ...
      const remainingDeadlineMs = (deadlineAt, fallbackMs = 1e3) => {
      ...
      const collectOpenShadowElements = (root, selector) => {
        ...
        const visit = (scope) => {
          ...
          for (const host2 of queryAll(scope, "*")) {
            if (!host2?.shadowRoot) continue;
            hostCount += 1;
            if (hostCount > 1800) break;
            visit(host2.shadowRoot);
          }
        };
        visit(root || document);
        return out;
...
${node.nodeValue}`;
      ...
      window.addEventListener(NOTION_SEND_TEXT_EVENT, async (event) => {
        ...
        window.postMessage({ source: NOTION_SEND_TEXT_SOURCE2, type: "response", id, data }, "*");
      ...
      window.addEventListener(NOTION_SEND_PROMPT_EVENT, async (event) => {
        ...
        window.postMessage({ source: NOTION_SEND_PROMPT_SOURCE2, type: "response", id, data }, "*");
      }, notionSendListenerOptions);
