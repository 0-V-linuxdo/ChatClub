(() => {
  ...
  var CONTENT_RUNTIME_SOURCE_SHA256 = "810966dcd834deb8121b679583437cc1a1948f1a056b17d3f312b297890e8207";
  ...
  var CONTENT_RUNTIME_IMPLEMENTATION_SHA256 = "10dd8712e4309734223d632a6de25477028d380005b9472ee28a5a458595c133";
  var CONTENT_RUNTIME_IMPLEMENTATION_VERSION = "2026.07.16.2+implementation.10dd8712e4309734223d632a6de25477028d380005b9472ee28a5a458595c133";
  ...
  function installGrokStorageAccessBridge(runtimes) {
    ...
    runtimes.register(runtimeName, {
      ...
      dispose() {
        ...
        reloadTimer = 0;
        try {
          if (permissionRecord) permissionRecord.onchange = null;
        } catch {
        }
  ...
  function installNativeCopyBridge(runtimes, copySource) {
    ...
    const blobText = async (blob) => {
      try {
        ...
        if (blob !== void 0 && blob !== null) return String(blob);
      } catch {
      }
      return "";
    };
    ...
    const dispatchSyntheticCopyEvent = () => {
      try {
        ...
        let event = null;
        try {
          const transfer = typeof DataTransfer === "function" ? new DataTransfer() : void 0;
    ...
    const messageListener = (event) => {
      ...
      if (message.action === "install") {
        ...
        const timeoutMs = Math.max(300, Number(message.data?.timeoutMs || message.timeoutMs || message.timeout || message.data?.timeout) || 5e3);
  ...
  function installDeepSeekDeleteBridge(runtimes, deleteSource) {
    ...
    const textOf = (el) => normalize2([
      el?.getAttribute?.("aria-label"),
      el?.getAttribute?.("title"),
      el?.innerText || el?.textContent || ""
    ].filter(Boolean).join(" "));
    const svgTextOf = (el) => normalize2(all("svg,use,path,circle", el).map((item) => [
      item.tagName,
    ...
    const disabled = (el) => {
      if (!el) return true;
      try {
        return el.disabled === true || el.getAttribute?.("disabled") != null || String(el.getAttribute?.("aria-disabled") || "").toLowerCase() === "true";
      } catch {
        return false;
      }
    };
    ...
    const fakeReactEvent = (target, currentTarget, type = "click") => {
      ...
      const event = {
        ...
        composedPath() {
          return eventPathFor(target, currentTarget);
        }
      };
    ...
    const activateUntil = async (node, getter, handlerNames, { settleMs = 180 } = {}) => {
      ...
      for (const attempt of attempts) {
        ...
        await wait2(Math.max(40, Number(settleMs) || 40));
        const value = read();
        if (value) return value;
      }
      return read();
    };
    const waitFor = async (getter, timeoutMs = 3e3, intervalMs = 90) => {
      const deadline = Date.now() + Math.max(0, Number(timeoutMs) || 0);
      while (Date.now() <= deadline) {
        const value = getter();
        if (value) return value;
        await wait2(Math.max(30, Number(intervalMs) || 30));
      }
      return getter();
    };
    ...
    const findTopicMoreButton = (link, hints = {}) => {
      ...
      const add = (node, extra = 0) => {
        ...
        const value = compact2(textOf(target));
        if (value && !/more|menu|options|ellipsis|dots|更多|菜单|选项/.test(value)) return;
        seen.add(target);
        candidates.push({
          element: target,
          score: extra + (visible(target) ? 180 : 40) + (!value ? 180 : 0) + Math.max(0, 100 - Math.abs((rect.left + rect.right) / 2 - (linkRect.right - 28))),
          right: rect.right
        });
      };
    ...
    const menuRoots = (hints = {}) => candidateNodes([
      ...
      return area < 45e4 && /delete|rename|pin|share|删除|重命名|置顶|分享/i.test(value);
    }).sort((a, b) => {
      const ar = rectOf(a);
      const br = rectOf(b);
    ...
    const dialogTextMatches = (value) => {
      ...
      const token = compact2(value);
      return /are you sure|chat can(?:'|’)?t be recovered|chat cant be recovered|share links from it will be disabled|recover|recovered|confirm|cancel|cannot be undone|permanently|确定|确认|取消|恢复|不可恢复|无法恢复|不能恢复/.test(text) || /areyousure|chatcantberecovered|sharelinksfromitwillbedisabled|recover|recovered|confirm|cancel|cannotbeundone|permanently|确定|确认|取消|恢复|不可恢复|无法恢复|不能恢复/.test(token);
    };
    const dialogConfirmContextMatches = (value) => {
      const text = String(value || "").toLowerCase();
      const token = compact2(value);
      return /are you sure|chat can(?:'|’)?t be recovered|chat cant be recovered|share links from it will be disabled|recover|recovered|confirm|cannot be undone|permanently|确定|确认|恢复|不可恢复|无法恢复|不能恢复/.test(text) || /areyousure|chatcantberecovered|sharelinksfromitwillbedisabled|recover|recovered|confirm|cannotbeundone|permanently|确定|确认|恢复|不可恢复|无法恢复|不能恢复/.test(token);
    };
    ...
    const dialogRoots = (hints = {}) => candidateNodes([
      ...
      if (!semantic && (area < 1200 || area > 38e4)) return false;
    ...
    const isConfirmText = (value) => {
      ...
      return token === "deletechat" || token === "delete" || token === "confirm" || token === "confirmdelete" || token === "删除" || token === "确认" || token === "确认删除" || token === "删除聊天";
    };
    ...
    const findConfirmButton = (hints = {}) => {
      ...
      for (const root of dialogRoots(hints)) {
        ...
        for (const node of candidateNodes("button,[role='button'],[tabindex]:not([tabindex='-1']),[class*='button' i],div", hints, "confirmCandidate", root)) {
          ...
          candidates.push({
            element: target,
            score: compact2(value) === "deletechat" || compact2(value) === "删除聊天" ? 600 : 0,
            area: rect ? rect.width * rect.height : 0,
            right: rect?.right || 0
          });
        }
  ...
  var deadlineExpired = (deadlineAt) => remainingDeadlineMs(deadlineAt, 1) <= 0;
  async function waitUntilDeadline(ms, deadlineAt) {
  ...
  var compact = (value) => normalize(value).toLowerCase().replace(/\s+/g, "");
  var withDeliveryState = (state, result) => ({ ...result, deliveryState: state });
  function createNotionSubmissionNavigation(sendId, method, href) {
    let url;
    try {
    ...
    const barrierState = validHost && path === "/ai" ? "required" : validHost && path === "/chat" && url.searchParams.get("t") ? "not-required" : validHost && path === "/chat" ? "required" : "unknown";
    return { sendId, appId: "NotionAI", initialHref: String(href || ""), barrierState, method };
  }

  // content-src/preload/notion-events.js
  function installNotionEventListeners(options = {}) {
  ...
  function createNotionAttachmentInspector(deps = {}) {
    ...
    const hasNotionUploadInProgress = (editor) => {
      ...
      return collectOpenShadowElements(scope, selector).some((element) => {
        ...
        const explicitProgress = ariaBusy || role === "progressbar" || tag === "progress" || /\b(?:uploading|upload progress|processing (?:file|image))\b/.test(haystack) || /正在(?:上传|处理)|上传(?:中|进度)/.test(haystack);
        ...
        for (let depth = 0; node && depth < 4; depth += 1) {
          ...
          const parentUploadContext = /\b(?:upload|uploading|attachment|file-preview|upload-preview|file|image|preview)\b/.test(parentHaystack) || /上传|附件|图片|图像|文件|预览/.test(parentHaystack);
          const parentExplicitProgress = String(node.getAttribute?.("aria-busy") || "").toLowerCase() === "true" || String(node.getAttribute?.("role") || "").toLowerCase() === "progressbar" || String(node.tagName || "").toLowerCase() === "progress" || /\b(?:uploading|upload progress|processing (?:file|image))\b/.test(parentHaystack) || /正在(?:上传|处理)|上传(?:中|进度)/.test(parentHaystack);
  ...
  function installNotionSendBridge(runtimes, protocol) {
    ...
    const NOTION_SEND_BRIDGE_VERSION = NOTION_SEND_TEXT_SOURCE2.split(":").at(-1);
    const existing = runtimes.registration(runtimeName);
    ...
    const notionSendRequestCache = /* @__PURE__ */ new Map();
    const notionSendRequestTimers = /* @__PURE__ */ new Set();
    const cleanup = () => {
      try {
        notionSendBridgeAbort.abort();
      } catch {
      }
    ...
    const collectOpenShadowElements = (root, selector) => {
      ...
      const visit = (scope) => {
        ...
        if (scope.nodeType === 1) {
          try {
            if (scope.matches?.(selector)) push(scope);
          } catch {
          }
        }
    ...
    const editorScope = (editor) => editor?.closest?.("form") || editor?.closest?.("[class*='composer' i],[class*='input' i],[data-testid*='composer' i]") || editor?.parentElement?.parentElement || editor?.parentElement || document.body;
    const editorText = (editor) => editor instanceof HTMLTextAreaElement || editor instanceof HTMLInputElement ? normalize(editor.value) : normalize(editor?.innerText || editor?.textContent || "");
    ...
    const promptReceiveFailureReason = (editor, value) => {
      return promptWhitespaceCollapsedMatches(editorText(editor), value) ? "Notion AI collapsed prompt whitespace/newlines before submit" : "Notion AI input did not receive the prompt";
    };
    const promptSubmitFailureReason = (editor, value) => {
      return promptWhitespaceCollapsedMatches(editorText(editor), value) ? "Notion AI collapsed prompt whitespace/newlines before submit" : "Notion AI kept the prompt in the composer after submit";
    };
    const findEditor = () => Array.from(document.querySelectorAll("div[contenteditable='true'][role='textbox'],div[contenteditable='true'],div[role='textbox'],textarea")).filter(visible).sort((a, b) => b.getBoundingClientRect().bottom - a.getBoundingClientRect().bottom)[0] || null;
    ...
    const waitFor = async (check, timeoutMs = 2e3, intervalMs = 80, deadlineAt = 0) => {
    ...
    const insertEditorHtml = async (editor, value) => {
      ...
      try {
        document.execCommand("insertHTML", false, escapeHtml(value).replace(/\n/g, "<br>"));
      } catch {
      }
    ...
    const liveEditor = async (fallback = null, timeoutMs = 3e3, deadlineAt = 0) => await waitFor(findEditor, timeoutMs, 100, deadlineAt) || fallback;
    ...
    const prepareComposerForRun = async (editor, deadlineAt = 0) => {
      ...
      editor = await liveEditor(editor, 3e3, deadlineAt);
    ...
    const ensurePromptCommitted = async (editor, text, deadlineAt = 0) => {
      if (!text) return { ok: true, editor: await liveEditor(editor, 2e3, deadlineAt) };
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        if (deadlineExpired(deadlineAt)) break;
        editor = await liveEditor(editor, attempt === 1 ? 3e3 : 4e3, deadlineAt);
        if (!editor) break;
...
${node.nodeValue}`;
    ...
    const isNotionComposerElement = (element, requireVisible = true) => {
      ...
      const tag = String(element.tagName || "").toLowerCase();
      const editable = tag === "textarea" || tag === "input" || String(element.getAttribute?.("contenteditable") || "").toLowerCase() === "true" || String(element.getAttribute?.("role") || "").toLowerCase() === "textbox";
    ...
    const focusNotionComposer = async (deadlineAt = 0) => {
      const composer = await waitFor(findEditor, 4e3, 120, deadlineAt);
    ...
    const findNotionComposerContainer = (composerEl) => {
      ...
      try {
        pushScope(composer?.closest?.("[data-testid*='chat' i],[data-testid*='composer' i],[data-testid*='prompt' i],[data-testid*='unified-chat' i]") || null);
      } catch {
      }
      try {
    ...
    const notifyNotionSendActivated = (payload = {}, method = "notion-submit") => {
      ...
      try {
        window.dispatchEvent(new CustomEvent(NOTION_SEND_ACTIVATED_EVENT2, {
    ...
    const sendNotionMessage = async (editor, deadlineAt = 0, payload = {}) => {
      ...
      if (button) {
        if (!isNotionSendButtonDisabled(button)) {
          ...
          const submissionNavigation2 = notifyNotionSendActivated(payload, "notion-button");
          if (clickElement(button)) return { ok: true, method: "notion-bridge-button", submissionNavigation: submissionNavigation2 };
          return {
            ok: false,
            deliveryState: "unknown",
            method: "notion-bridge-button",
            reason: "Notion AI submit activation failed",
            submissionNavigation: submissionNavigation2
          };
        }
      }
      if (deadlineExpired(deadlineAt)) return { ok: false, method: "notion-bridge", reason: "Send deadline exceeded" };
    ...
    const pressEnter = async (editor) => {
      ...
      for (const type of ["keydown", "keypress", "keyup"]) {
        try {
          editor?.dispatchEvent(new KeyboardEvent(type, init));
        } catch {
        }
        await wait(45);
      }
    ...
    const promptFiles = (images) => (Array.isArray(images) ? images : []).map((entry, index) => dataUrlToFile(entry, index)).filter((file) => file && String(file.type || "").startsWith("image/"));
    const createTransfer = (files = [], textValue = "") => {
      try {
        ...
        if (textValue) {
          try {
            transfer.setData("text/plain", String(textValue));
          } catch {
          }
        }
        return transfer;
    ...
    const waitForNotionAttachmentChange = async (editor, previousSnapshot, timeoutMs = 9e3, deadlineAt = 0) => {
      const start = Date.now();
    ...
    const waitForNotionImagesReady = async (editor, { requireImage = true, minAttachments = 0, timeoutMs = 45e3, intervalMs = 160, settleMs = 600, deadlineAt = 0 } = {}) => {
      ...
      const observed = await waitForStableNotionState({
        ...
        isSatisfied: (state2) => Boolean(state2?.ok),
        timeoutMs,
        intervalMs,
        settleMs,
        deadlineAt
    ...
    const waitForNotionReadyToSend = async (editor, { requireImage = false, minAttachments = 0, timeoutMs = 45e3, intervalMs = 160, settleMs = 600, deadlineAt = 0 } = {}) => {
      let composerRef = resolveNotionComposerElement(editor, { requireVisible: true }) || await focusNotionComposer(deadlineAt);
    ...
    const commitPastedTextEarly = async (editor, textValue, deadlineAt = 0) => {
      ...
      editor = await liveEditor(editor, 1e3, deadlineAt);
    ...
    const clearAttachments = async (editor) => {
      ...
      try {
        ...
        const seen = /* @__PURE__ */ new Set();
        const buttons = collectOpenShadowElements(scope, selector).map((element) => {
          try {
    ...
    const attachImagesWithRetries = async (editor, files, retryCount = 3, deadlineAt = 0, textValue = "") => {
      ...
      for (let attempt = 1; attempt <= attempts; attempt += 1) {
        ...
        if (attempt > 1) {
          editor = await prepareComposerForRun(await liveEditor(editor, 3e3, deadlineAt), deadlineAt);
        ...
        const accepted = await waitForNotionAttachmentChange(editor, baseline, Math.min(9e3, remainingDeadlineMs(deadlineAt, 9e3)), deadlineAt);
        ...
        const ready = await waitForNotionImagesReady(editor, {
          ...
          timeoutMs: Math.min(45e3, remainingDeadlineMs(deadlineAt, 45e3)),
          intervalMs: 160,
          settleMs: 600,
          deadlineAt
        });
    ...
    const runNotionSendOnce = async (payload, scope, runner) => {
      ...
      const existing2 = notionSendRequestCache.get(key);
      if (existing2) return existing2.promise;
      const record = { promise: null };
      record.promise = Promise.resolve().then(runner).then((result) => {
        record.promise = Promise.resolve(result);
        forgetNotionSendRequest(key, record, 12e4);
        return result;
      }, (error) => {
        forgetNotionSendRequest(key, record, 3e4);
        throw error;
      });
    ...
    const sendNotionText = async (payload = {}) => {
      const deadlineAt = deadlineFromPayload(payload, 1e4);
      ...
      let editor = await waitFor(findEditor, 3e3, 100, deadlineAt);
      ...
      const readyToSend = await waitForNotionReadyToSend(editor, {
        requireImage: false,
        timeoutMs: Math.min(1e4, remainingDeadlineMs(deadlineAt, 1e4)),
        intervalMs: 160,
        settleMs: 300,
        deadlineAt
      });
      ...
      if (!sent.ok) {
        return withDeliveryState(sent.deliveryState === "unknown" ? "unknown" : "not-sent", {
          ...
          sent: false,
          method: "notion-bridge",
          reason: sent.reason || "Notion AI submit failed",
      ...
      if (await waitFor(() => submitted(sendEditor, text, beforeOutsideCount), 2600, 100, deadlineAt)) {
        return withDeliveryState("sent", {
          ok: true,
          sent: true,
          method: sent.method || "notion-bridge-button",
          verified: true,
          submissionNavigation: sent.submissionNavigation
        });
      }
    ...
    const sendNotionPrompt = async (payload = {}) => runNotionSendOnce(payload, "prompt", async () => {
      const deadlineAt = deadlineFromPayload(payload, 6e4);
      const text = String(payload.text || "").trim();
      ...
      if (files.length) {
        ...
        editor = await liveEditor(attached.editor || editor, 4e3, deadlineAt);
      }
      if (text) {
        ...
        editor = await liveEditor(committedAfterImages.editor, 4e3, deadlineAt);
        ...
        editor = await liveEditor(committedBeforeSend.editor, 4e3, deadlineAt);
      }
    ...
    installNotionEventListeners({
      ...
      textEvent: NOTION_SEND_TEXT_EVENT2,
      promptEvent: NOTION_SEND_PROMPT_EVENT2,
      textSource: NOTION_SEND_TEXT_SOURCE2,
      promptSource: NOTION_SEND_PROMPT_SOURCE2,
  ...
  function installPreload() {
    ...
    function installChatClubWebviewShim() {
      ...
      try {
        ...
        window.close = function() {
          ...
          try {
            return nativeClose.call(window);
          } catch {
          }
        };
      } catch {
    ...
    function preferredModelFocusShieldLease() {
      let raw = "";
      try {
        raw = String(document.documentElement?.getAttribute(PREFERRED_MODEL_FOCUS_SHIELD_ATTRIBUTE) || "");
      } catch {
      }
      if (!raw) return null;
      try {
    ...
    function installPreferredModelNavigationFocusGuardBridge() {
      const prepare = (message = {}) => {
        ...
        const expiresAt = Number.isFinite(requestedExpiresAt) && requestedExpiresAt > now ? Math.min(requestedExpiresAt, now + NAVIGATION_FOCUS_GUARD_LEASE_MS) : now + NAVIGATION_FOCUS_GUARD_LEASE_MS;
        let stored = false;
        try {
        ...
        if (stored && message.phase !== "adopt") {
          ...
          try {
            const rawName = String(window.name || "");
            const cleanName = rawName.replace(/(?:^|&)chatclub_focus_guard_until=\d+&chatclub_focus_guard_token=[^&]+$/, "").replace(/(?:^|&)chatclub_focus_guard_until=\d+$/, "");
    ...
    function installPreferredModelFocusShield() {
      ...
      const currentElementFocus = (() => {
        try {
          return Object.getOwnPropertyDescriptor(HTMLElement.prototype, "focus")?.value;
        } catch {
          return null;
        }
      })();
      ...
      try {
        currentWindowFocus = window.focus;
      } catch {
      }
      if (previous?.version === PREFERRED_MODEL_FOCUS_SHIELD_VERSION && currentElementFocus === previous.guardedElementFocus && currentWindowFocus === previous.guardedWindowFocus) {
        try {
      ...
      try {
        elementFocusDescriptor = Object.getOwnPropertyDescriptor(HTMLElement.prototype, "focus");
      } catch {
      }
      try {
        windowFocusDescriptor = Object.getOwnPropertyDescriptor(window, "focus");
      } catch {
      }
      ...
      const pageCaretTrustedRecently = () => Boolean(pageCaretTrustedAt && Date.now() - pageCaretTrustedAt < 1e3);
      ...
      try {
        guardedWindowFocus.toString = () => nativeWindowFocus.toString();
      } catch {
      }
      ...
      try {
        const descriptor = windowFocusDescriptor && Object.prototype.hasOwnProperty.call(windowFocusDescriptor, "value") ? { ...windowFocusDescriptor, value: guardedWindowFocus } : { configurable: true, enumerable: windowFocusDescriptor?.enumerable ?? true, writable: true, value: guardedWindowFocus };
      ...
      const refreshLease = () => {
        ...
        const delay = Math.min(2147483e3, Math.max(1, lease.expiresAt - Date.now() + 25));
    ...
    function installMainWorldLocationBridge() {
      ...
      const notify = (kind, force = false) => {
        ...
        try {
          window.postMessage({
            source: MAIN_WORLD_LOCATION_SOURCE2,
            type: "notification",
    ...
    function installGeminiModelPickerBridge() {
      ...
      const listenerRecordFor = (button) => {
        const host2 = button?.closest?.("gem-button, [data-test-id='bard-mode-menu-button'], .gds-mode-switch-button") || button;
        for (let index = records.length - 1; index >= 0; index -= 1) {
          const record = records[index];
          const target = record.target;
          if (!target?.isConnected) continue;
          if (target === host2 || target === button || target.contains?.(button) || host2?.contains?.(target)) return record;
        }
      ...
      const modelClickEvent = (button, currentTarget) => {
        ...
        return {
          ...
          composedPath() {
            return eventPathFor(button, currentTarget);
          }
        };
      };
      ...
      const stats = previousBridge?.stats && typeof previousBridge.stats === "object" ? previousBridge.stats : { requests: 0, invocations: 0 };
      let activeRunId = "";
      let activeRunToken = "";
      let activeRunGeneration = 0;
      const documentRunToken = () => {
        try {
          return String(document.documentElement?.getAttribute(GEMINI_MODEL_PICKER_RUN_TOKEN_ATTRIBUTE) || "");
        } catch {
          return "";
        }
      };
      ...
      const runIsCurrent = (run) => Boolean(
        run?.runId && run?.runToken && documentRunToken() === run.runToken && activeRunId === run.runId && activeRunToken === run.runToken && activeRunGeneration === run.runGeneration
      );
      const acceptRun = (run) => {
      ...
      const messageListener = (event) => {
        const message = event.data;
        if (message?.source !== GEMINI_MODEL_PICKER_SOURCE2 || message.type !== "request" || !["open", "cancel"].includes(message.action)) return;
