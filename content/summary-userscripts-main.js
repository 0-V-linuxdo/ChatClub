(() => {
  ...
  function createSummaryRunnerRegistry() {
    ...
    scripts["manus"] = async function(api) {
      ...
      const qs2 = (selector, scope = document) => {
        try {
      ...
      const svgSignature2 = (node) => normalize2([
        node,
        ...qsa2("svg,title,desc,path,rect,line,polyline,polygon,use,img,[data-icon],[class]", node).slice(0, 80)
      ].map((el) => [
        classText2(el),
        el && el.getAttribute && el.getAttribute("data-icon"),
        el && el.getAttribute && el.getAttribute("aria-label"),
      ...
      const useful = (value) => {
        const text2 = normalize2(value);
        if (!text2 || text2.length < 2 || text2.length > 2e5) return "";
        if (/^(?:copy|copied|copy link|retry|edit|share|task completed|how was this result|复制|已复制|重试|编辑|分享|任务已完成)$/i.test(text2)) return "";
        return text2;
      };
      ...
      const actionRow = (button) => {
        ...
        for (let i = 0; i < 5 && node && node !== chatRoot(); i += 1) {
          const companions = qsa2("button,[role=button]", node).filter((item) => item !== button && canonicalControl(item) && (assistantAction(item) || userAction(item)));
      ...
      const siblingActions = (button) => qsa2("button,[role=button]", actionRow(button)).filter((item) => item !== button);
      const messageOwner = (button) => {
        const eventNode = closest2(button, "[data-event-id]");
      ...
      const domFallback = (root2, copiedAssistant = "") => {
        ...
        const assistantHint = normalize2(copiedAssistant).slice(0, 180);
...
${raw.slice(-36e3)}`;
  ...
  function installSummaryMainRuntime() {
    ...
    const CONTENT_RUNTIME_IDENTITY2 = createContentRuntimeBundleIdentity(CONTENT_RUNTIME_SUMMARY_MAIN_BUNDLE_IDENTITY);
    ...
    }
    {
      ...
      runtimes.register("summary-page", {
        ...
        activate() {
          window[CUSTOM_SUMMARY_EXECUTOR2] = customSummaryExecutor;
          window.addEventListener("message", onSummaryPageMessage, true);
        },
        dispose() {
          window.removeEventListener("message", onSummaryPageMessage, true);
          if (window[CUSTOM_SUMMARY_EXECUTOR2] === customSummaryExecutor) delete window[CUSTOM_SUMMARY_EXECUTOR2];
        }
      });
    }
  }
