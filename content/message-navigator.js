(() => {
  ...
  function installMessageNavigator() {
    ...
    const EFFECT_MODES = /* @__PURE__ */ new Set(["none", "border", "pulse", "fade", "jiggle"]);
...
${ancestor.text.toLowerCase().replace(/\s+/g, " ").slice(0, 500)}`;
    ...
    function containsComposerChrome(element) {
      if (!element || element.nodeType !== 1) return false;
      const controls = safeQsa(COMPOSER_CHROME_SELECTOR, element).filter((node) => node !== element && visible(node) && !safeClosest(node, `#${ROOT_ID}`));
    ...
    function containsMultipleMessageRoots(element) {
      const roots = safeQsa(STRICT_MESSAGE_ROOT_SELECTOR, element).filter((node) => node !== element && visible(node) && !safeClosest(node, `#${ROOT_ID}`));
...
${normalize(item.text).toLowerCase().slice(0, 260)}`;
    ...
    function adapterBaseItems(config, adapter = {}) {
      ...
      return dedupeItems(adapterBaseQuery(config).map((element) => {
        ...
        const rawText2 = adapter.text?.(element, config, { role, target, textSource }) || cloneText(textSource, cleanupSelectors) || cloneText(element, cleanupSelectors);
        ...
        return {
          ...
          effectTarget,
          role,
          text: compactText(rawText2, config.summaryMaxChars)
        };
      }));
    }
    ADAPTERS.generic = {
    ...
    ADAPTERS.notion = {
      ...
      effectTarget(element, config, context = {}) {
        ...
        return effectMatches(root || element, ".markdown,[class*='message' i],[role='article']", config, { prefer: "largest" }) || root || context.target || element;
      },
      collect(config) {
    ...
    ADAPTERS.gemini = {
      ...
      effectTarget(element, config, context = {}) {
        const root = messageRootForEffect(element, context.target, config, { allowBroad: true });
        if (context.role === "user") {
          return effectMatches(root || element, ".user-query-bubble-with-background,.query-text,.query-content,.user-query-container", config) || context.target || root || element;
        }
        return effectMatches(root || element, ".model-response-text .markdown,message-content .markdown,.model-response-text,message-content,.response-container-content,.response-container,structured-content-container", config, { prefer: "largest" }) || root || context.target || element;
      },
    ...
    ADAPTERS.kagi = {
      ...
      effectTarget(element, config, context = {}) {
        ...
        if (effectRole === "user") {
          return (usableEffectTarget(root, config, { allowBroad: true, role: effectRole }) ? root : null) || effectMatches(root || element, ".chat_bubble_content,.markdown", config, { role: effectRole, prefer: "largest" }) || context.target || element;
        }
        return (usableEffectTarget(root, config, { allowBroad: true, role: effectRole }) ? root : null) || effectMatches(root || element, ".chat_bubble_content,.markdown", config, { prefer: "largest", role: effectRole }) || context.target || element;
      },
      collect(config) {
